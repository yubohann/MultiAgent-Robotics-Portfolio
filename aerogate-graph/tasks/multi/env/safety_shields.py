"""Action-shield and velocity-projection helpers for ``MultiGate2DEnv``."""
...
class SafetyShieldRuntimeMixin:
    """Velocity-level action safety shields."""

    def _default_action_shield_info(self) -> dict[str, object]:
        return {
            "enabled": bool(self.env_config.action_safety_shield_enabled),
            "active": False,
    ...
    def _apply_action_safety_shield(
        ...
        dynamic_gate_task_threshold = float(
            self.env_config.formation_line_collapse_task_ratio
        )
    ...
    def _apply_post_gate_cruise_velocity_floor(
        ...
        *,
        ...
        if (
            ...
            or not bool(self.env_config.action_safety_shield_post_gate_cruise_enabled)
        ...
        min_forward_mps = max(
            float(self.env_config.action_safety_shield_post_gate_cruise_min_forward_mps),
            0.0,
        )
        ...
        gate_behind_m = max(
            float(self.env_config.action_safety_shield_post_gate_cruise_gate_behind_m),
            0.0,
        )
        ...
        goal_margin_m = max(
            float(self.env_config.action_safety_shield_post_gate_cruise_goal_margin_m),
            0.0,
        )
        ...
        min_pair_required_m = max(
            float(self.env_config.action_safety_shield_post_gate_cruise_min_pair_distance_m),
            0.0,
        )
        ...
        min_clearance_required_m = max(
            float(self.env_config.action_safety_shield_post_gate_cruise_min_clearance_m),
            0.0,
        )
    ...
    def _apply_pairwise_velocity_shield(
        ...
        *,
        ...
        activation_margin_m = max(
            float(self.env_config.action_safety_shield_separation_margin_m),
            0.0,
        )
        ...
        brake_scale = max(float(self.env_config.action_safety_shield_brake_scale), 0.0)
        repulsion_scale = max(
            float(self.env_config.action_safety_shield_repulsion_scale),
            0.0,
        )
        outward_bias_scale = max(
            float(self.env_config.action_safety_shield_outward_slot_bias_scale),
            0.0,
        )
        pair_closing_brake_only = bool(
            self.env_config.action_safety_shield_pair_closing_brake_only
        )
        pair_time_horizon_s = max(
            float(self.env_config.action_safety_shield_pair_time_horizon_s),
            0.0,
        )
        priority_team_size_limit = max(
            int(self.env_config.action_safety_shield_priority_team_size_limit),
            1,
        )
    ...
    def _apply_boundary_velocity_shield(
        ...
        *,
        ...
        boundary_margin_m = max(
            float(self.env_config.action_safety_shield_boundary_margin_m),
            0.0,
        )
        brake_scale = max(
            float(self.env_config.action_safety_shield_boundary_brake_scale),
            0.0,
        )
        inward_scale = max(
            float(self.env_config.action_safety_shield_boundary_inward_scale),
            0.0,
        )
    ...
    def _apply_guidance_corridor_velocity_shield(
        ...
        *,
        ...
        corridor_margin_m = max(
            float(self.env_config.action_safety_shield_guidance_margin_m),
            0.0,
        )
        inward_scale = max(
            float(self.env_config.action_safety_shield_guidance_inward_scale),
            0.0,
        )
    ...
    def _apply_dynamic_gate_channel_velocity_shield(
        ...
        *,
        ...
        if (
            not self._dynamic_gate_enabled
            or not self._dynamic_gates
            or not bool(self.env_config.action_safety_shield_gate_channel_enabled)
            or positions_xy.size == 0
            or commanded_velocities_xy.size == 0
        ):
            return commanded_velocities_xy.copy(), 0.0
        ...
        lookahead_m = max(
            float(self.env_config.action_safety_shield_gate_channel_lookahead_m),
            0.0,
        )
        behind_m = max(
            float(self.env_config.action_safety_shield_gate_channel_behind_m),
            0.0,
        )
        lateral_gain = max(
            float(self.env_config.action_safety_shield_gate_channel_lateral_gain),
            0.0,
        )
        max_lateral_mps = max(
            float(self.env_config.action_safety_shield_gate_channel_max_lateral_mps),
            0.0,
        )
        slowdown_scale = max(
            float(self.env_config.action_safety_shield_gate_channel_slowdown_scale),
            0.0,
        )
        ...
        gate_half_width_m = float(self._dynamic_gate_config.gate_half_width_m)
        gate_post_radius_m = float(self._dynamic_gate_config.gate_post_radius_m)
    ...
    def _apply_obstacle_velocity_shield(
        ...
        *,
        ...
        margin_m = max(
            float(self.env_config.action_safety_shield_obstacle_margin_m),
            0.0,
        )
        brake_scale = max(
            float(self.env_config.action_safety_shield_obstacle_brake_scale),
            0.0,
        )
        repulsion_scale = max(
            float(self.env_config.action_safety_shield_obstacle_repulsion_scale),
            0.0,
        )
        horizon_s = max(
            float(self.env_config.action_safety_shield_obstacle_time_horizon_s),
            0.0,
        )
        if (
            margin_m <= 0.0
        ...
        for agent_idx, position_xy in enumerate(np.asarray(positions_xy, dtype=np.float32)):
            ...
            for obstacle in obstacles:
                ...
                obstacle_velocity_xy = np.asarray(obstacle.velocity_xy, dtype=np.float32)
                ...
                if str(obstacle.species) == "dynamic_gate_post":
