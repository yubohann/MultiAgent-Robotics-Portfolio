"""Dynamic-gate layout, cache, and live obstacle-map helpers for ``MultiGate2DEnv``."""
...
class DynamicGateRuntimeMixin:
    """Dynamic gate layout, caches, and live obstacle maps."""
    ...
    def _reset_dynamic_gate_layout(self, *, seed: int | None) -> None:
        ...
        static_layout = (
            float(self._dynamic_gate_config.moving_gate_speed_mps) <= 1.0e-6
            or float(self._dynamic_gate_config.moving_gate_amplitude_m) <= 1.0e-6
        )
        self._dynamic_gates = generate_gate_layout(
            gate_count=int(self._dynamic_gate_config.gate_count),
            seed=layout_seed,
    ...
    def _dynamic_gate_centers_xy(self, *, next_frame: bool = False) -> np.ndarray:
        ...
        centers = live_gate_centers(
            ...
            amplitude_m=float(self._dynamic_gate_config.moving_gate_amplitude_m),
            speed_mps=float(self._dynamic_gate_config.moving_gate_speed_mps),
            config=self._dynamic_gate_config,
        )
    ...
    def _dynamic_gate_obstacle_map(self) -> GateObstacleMap2D:
        ...
        obstacles = tuple(
            ...
            for post_xy, velocity_xy in zip(posts_xy, post_velocities_xy, strict=False)
        )
