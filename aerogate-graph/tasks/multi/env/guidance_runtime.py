"""Route-guidance query and fallback helpers for ``MultiGate2DEnv``."""
...
class GuidanceRuntimeMixin:
    """Route guidance lookup with heuristic fallback."""

    def _guidance_runtime_active(self) -> bool:
        reasoning = self.multi_config.reasoning
        return bool(
            reasoning.route_guidance_enabled
            or reasoning.guidance_shadow_mode
        )

    def _route_guidance_visible(self) -> bool:
        return bool(self.multi_config.reasoning.route_guidance_enabled)
    ...
    def _default_route_guidance_meta(
        ...
        *,
        ...
        reasoning = self.multi_config.reasoning
        return {
            "provider": str(reasoning.guidance_provider),
            "model_name": str(reasoning.guidance_model_name),
            "prompt_version": str(reasoning.guidance_prompt_version),
            "source": str(source),
            "latency_ms": None,
            "cache_hit": False,
    ...
    def _resolve_guidance_query_interval_steps(self) -> int:
        budget_hz = float(self.multi_config.reasoning.inference_budget_hz)
        if budget_hz <= 0.0:
            return 0
    ...
    def _build_guidance_query_payload(
        ...
        *,
        ...
        return {
            "prompt_version": str(self.multi_config.reasoning.guidance_prompt_version),
            "stage_name": str(self.multi_config.reasoning.guidance_stage_name or self.multi_config.paper_variant),
            "scene_mode": str(self.multi_config.scene.scene_mode),
