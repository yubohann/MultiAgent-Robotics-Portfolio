"""Reward info, clearance, collision, and formation-status helpers for ``MultiGate2DEnv``."""
...
class RewardRuntimeMixin:
    """Info payloads, clearance, collision, and formation status."""
    ...
    def _build_info(
        ...
        *,
        ...
        return {
            ...
            "gate_bottom_height_m": float(self._dynamic_gate_config.gate_opening_bottom_height_m),
            "gate_top_height_m": float(self._dynamic_gate_config.gate_opening_top_height_m),
            "gate_center_height_m": float(self._dynamic_gate_config.gate_center_height_m),
            ...
            "moving_gate_speed_mps": float(self._dynamic_gate_config.moving_gate_speed_mps),
            "moving_gate_amplitude_m": float(
                self._dynamic_gate_config.moving_gate_amplitude_m
            ),
            ...
            "preparation_hold_mode": bool(self.env_config.preparation_hold_mode),
            "timeout_counts_as_success": bool(self.env_config.timeout_counts_as_success),
            ...
            "guidance_shadow_mode": bool(self.multi_config.reasoning.guidance_shadow_mode),
    ...
    def _route_plan_guidance_summary(
        ...
        if not bool(self.multi_config.reasoning.global_planner_enabled):
            return None
        path_waypoints = self._plan.waypoints_xy
    ...
    def _formation_shape_status(
        ...
        *,
        ...
        required_bands = int(self.env_config.formation_line_collapse_min_lateral_bands)
        ...
        active = bool(
            ...
            and (
                task_ratio >= float(self.env_config.formation_line_collapse_task_ratio)
                or corridor_inside
            )
        )
        ...
        band_width = max(float(self.env_config.formation_line_collapse_band_width_m), 1.0e-6)
