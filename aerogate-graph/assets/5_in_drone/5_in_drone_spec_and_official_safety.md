# 5_in_drone Asset Notes and Official Safety Distance Basis

Generated on 2026-05-04

This document describes the model position, geometry, mass and inertia, actuators and simulation configuration of the drone under `assets/5_in_drone`. It also records the official regulatory basis for the safe distance between two drones. The official safety distance stands apart from the project training rewards, the avoidance shield and the formation spacing parameters.

## 1. Model Position

`5_in_drone` is a custom 5-inch class quadrotor racing drone simulation asset. A full commercial hardware BOM for a DJI or PX4 model sits outside its scope. The `5_in` name comes from the asset directory and file names and marks a 5-inch propeller class FPV racing quad. The `.dae` meshes in the current checkout are Git LFS pointers, so the 5-inch propeller diameter serves as a naming label rather than a value measured from mesh vertices.

The asset breaks into these files.

| File | Purpose |
| --- | --- |
| `5_in_drone.usd` | Top-level USD that references and composes the sensor and physics layers |
| `configuration/5_in_drone_base.usd` | Geometry and appearance layer, about 102.6 MB |
| `configuration/5_in_drone_physics.usd` | Physics layer with rigid body, joint, mass and collision approximation data |
| `configuration/5_in_drone_sensor.usd` | Sensor layer, readable tokens show one `a_5_in_drone` Xform |
| `urdf/5_in_drone.urdf` | URDF source model with the clearest record of mass, inertia and the four prop and motor positions |
...
| USD path | `assets/5_in_drone/5_in_drone.usd` |
...
`assets/five_in_drone_graph_masac_training_backup.py` is the training backup config. It loads `configuration/5_in_drone_physics.usd` directly and disables contact sensors to isolate PhysX contact reporting cost in multi-agent Graph-MASAC training.
...
The readable asset information shows one Xform in `5_in_drone_sensor.usd`. Camera, IMU, GPS, barometer, ESC, battery, flight controller, video link and receiver hardware parameters sit outside the recorded tokens.
...
For simulation papers and training notes about `5_in_drone`, keep the official safety distance and the project safety distance separate in the writeup.
