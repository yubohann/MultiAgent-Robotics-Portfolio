...
This repository tracks code, configuration, and documentation. Obtain each dataset from its legitimate source and review its usage terms before running an experiment. Store all source assets below the local `data/` directory.
...
| AMLSim | `data/amlsim/outputs/<simulation>/` | Generated `accounts.csv`, transaction CSV, and alert and SAR files from an AMLSim simulation. |
...
- Publication scope covers summaries, reports, and manifests. Large generated files from the previous workspace stay in local storage.
- IEEE-CIS, Elliptic, AMLSim, and on-chain source data differ in license, preprocessing, and scale. Cross-dataset comparisons require the dataset-specific protocol.
- The training code supports deterministic seeds. GPU kernels and third-party libraries can introduce nondeterminism.
