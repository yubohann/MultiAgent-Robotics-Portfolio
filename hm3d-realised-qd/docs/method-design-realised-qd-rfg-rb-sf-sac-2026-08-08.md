...
状态，设计规格，Design Spec。实现差距见 §10。所有符号与现有 exploration 协议
hm3d-public-candidate-pool-v7，pass_through v5 transit，isaac-so3-feedback-v6，geometry–P06 冻结，兼容。
...
或直接复用片段，学习系统会把计划中的收益当成已发生的经验，虚假多样性与负迁移。
...
- 意图多样性增强与 QD 意图审计在层1/层2 范围执行，候选池可用性检查
  审计失败时按 §4.5 优雅回退，安全合同保持冻结。
...
- 意图多样性增强，已实现，`build_public_candidate_pool`，当可行候选的
  计划意图 cell 覆盖不足，任一轴 < 2 bins 或联合 < 6 cells，继续计分其余可行
  分配并做覆盖优先修剪。安全与质量准入保持不变。
- 池上限 K，正式实验固定 K=16。
...
3. 排除项，碰撞恢复，翻译复制轨迹，无新自由体素，stationarity 待通过
...
最近的 cell 的 realised 分布。**预测器不引入真值，不读计分器几何。**
...
同 exploration 合同校验。`MINIMUM_OUTCOME_ARCHIVE_ENTRIES_FOR_SELECTION=6` 前
...
### 7.1 场景与划分，P05 冻结
- train 00626，00459，已资格化，+ 后续按需扩充，00770 等，需完整 geometry–P06 资格链
- validation 00803，已资格化
- test P05 冻结的 test 集，00810+，计分前保持封存
- 正式主表场景数，≥3 train，至少 00626/00459 + 1 新增。扩充受限时
  主表只报告 2 场景并声明局限。
...
| gvp_mrep_port | 动态拓扑图 Voronoi 分区，受控迁移，| 强规划基线，声明非原 ROS 重跑，|
...
| 第 3 个 train 场景资格 | 待启动，00770 候选，| 补 geometry–P06 + 审计 |
