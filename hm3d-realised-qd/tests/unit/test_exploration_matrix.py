...
from realised_qd.evaluation.hm3d_exploration_matrix import (
...
def _raw(method: str, *, metric: float = 0.25, timeout: bool = False) -> dict[str, object]:
    public_context = {
        ...
        "context_id": "exploration-paired-context",
        "episode_id": "exploration-paired-episode",
    ...
    payload: dict[str, object] = {
        ...
        "public_episode_id": "exploration-paired-episode",
...
def _probes() -> tuple[ExplorationProbeRecord, ...]:
    return tuple(
        ExplorationProbeRecord.from_raw(method, _raw(method, metric=0.1 + index * 0.05))
        for index, method in enumerate(TASK_VALIDITY_METHODS)
    )


def test_exploration_pilot_uses_complete_public_paired_order_and_cannot_close() -> None:
    payload = assemble_exploration_task_validity_pilot(
        _probes(),
        matrix_run_id="exploration-pilot-001",
    ...
    assert payload["status"] == "EXPLORATION_TASK_VALIDITY_PILOT_COMPLETE"
    assert payload["exploration_task_validity_closed"] is False
...
def test_exploration_pilot_rejects_task_identity_drift() -> None:
    probes = list(_probes())
    changed = deepcopy(_raw("frontier_3d"))
    changed["public_candidate_pool_id"] = "1" * 64
    changed["runtime_record_id"] = "record-test"
    probes[1] = ExplorationProbeRecord.from_raw("frontier_3d", changed)
    with pytest.raises(ValueError, match="not a paired task matrix"):
        assemble_exploration_task_validity_pilot(
            probes,
            matrix_run_id="exploration-pilot-drift",
            sensor_profile_id="e" * 64,
            communication_contract_id="exploration-test-intermittent",
        )


def test_exploration_rejects_private_target_geometry() -> None:
    raw = _raw("random")
    ...
    with pytest.raises(ValueError, match="forbidden private field"):
        ExplorationProbeRecord.from_raw("random", raw)


def test_exploration_rejects_without_id_public_context() -> None:
    raw = _raw("random")
...
def test_failed_timeout_remains_an_episode_denominator() -> None:
    ...
    raw["status"] = "EXPLORATION_EXECUTION_SMOKE_FAILED"
    raw["runtime_record_id"] = "record-test"
    record = ExplorationProbeRecord.from_raw("random", raw)
...
def test_exploration_rejects_single_decision_engineering_smoke() -> None:
    raw = _raw("random")
...
def test_exploration_rejects_a_worker_that_did_not_use_its_physical_time_budget() -> None:
    raw = _raw("random")
...
def test_exploration_keeps_a_outcome_backed_early_terminal_failure_in_the_denominator() -> None:
    raw = _raw("random")
    raw["status"] = "EXPLORATION_EXECUTION_SMOKE_FAILED"
    raw["decision_count"] = 1
    raw["elapsed_physics_s"] = 10.21
