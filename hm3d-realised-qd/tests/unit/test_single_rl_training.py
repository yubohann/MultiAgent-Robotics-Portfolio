...
from realised_qd.adapters.hm3d_marvel import (
...
from realised_qd.evaluation.hm3d_communication_contract import (
    HM3DCommunicationContract,
)
from realised_qd.evaluation.hm3d_marl_ipp_training import (
    sample_from_exploration_training_record as sample_marl_ipp_training_record,
)
from realised_qd.evaluation.hm3d_marvel_training import (
    sample_from_exploration_training_record as sample_marvel_training_record,
)
from realised_qd.evaluation.hm3d_outcome_dataset import (
...
def _split_manifest() -> dict[str, object]:
    assignments = [
        {"scene_id": "00000-kfPV7w3FaU5", "split": "train", "asset_id": "a" * 64},
        {"scene_id": "00002-FxCkHAfgh7A", "split": "train", "asset_id": "b" * 64},
        {"scene_id": "00807-rsggHU7g7dh", "split": "validation", "asset_id": "c" * 64},
    ]
    return {
        "scene_assignments": assignments,
        "split_manifest_id": "p05-split-test",
    }
...
def _communication_contract() -> dict[str, object]:
    return {
        ...
        "contract_id": "exploration-test-intermittent",
...
def _raw(
    *,
    ...
    public_context = {
        "schema_version": "realised-qd-hm3d-exploration-v1",
    ...
    selection = {
        "strategy": strategy,
        "runner_version": "hm3d-runner-current",
        "selected_candidate_id": f"candidate-{action}",
        "selected_manifest_id": ("d" if action == 0 else "e") * 64,
    }
    execution = {
        ...
        "manifest_id": selection["selected_manifest_id"],
        "outcome_ids": [("1" if action == 0 else "2") * 64],
        "trace_ids": [("4" if action == 0 else "5") * 64],
        "total_energy_used_j": 12.5 + action,
    }
    second_context_id = f"{episode_id}:decision1"
    second_pool_id = "a" * 64
    emitted: dict[str, object] = {
        "schema_version": "hm3d-single-rl-train-transition-v4",
        "claim_limit": "test-only public decision transition",
        "scene_id": scene_id,
        "decision_id": "decision0",
        "public_context_id": public_context["context_id"],
        "public_candidate_pool_id": "pool-0",
        **schema_fields,
        ...
        "selected_manifest_id": selection["selected_manifest_id"],
        "reward_explored_free_flight_volume_auc_time_contribution": 0.10,
        "cost_energy_j": execution["total_energy_used_j"],
        "duration_s": 20.0,
        "terminated": False,
        "truncated": False,
        "next_public_context_id": second_context_id,
        "next_public_candidate_pool_id": second_pool_id,
        ...
        "outcome_id": "|".join(str(value) for value in execution["outcome_ids"]),
    }
    emitted["transition_id"] = f"{episode_id}:decision0"
    second_selection = {
        "strategy": strategy,
        "selected_candidate_id": "candidate-later",
        "selected_manifest_id": "f" * 64,
    }
    second_execution = {
        "evidence_class": "real_isaac_physx_cf2x",
        "manifest_id": "f" * 64,
    ...
    emitted_second.update(
        {
            ...
            "public_context_id": second_context_id,
            "public_candidate_pool_id": second_pool_id,
            "selected_action_index": 1,
            "selected_candidate_id": second_selection["selected_candidate_id"],
            "selected_manifest_id": second_selection["selected_manifest_id"],
            ...
            "next_public_context_id": second_context_id,
            "next_public_candidate_pool_id": second_pool_id,
            "outcome_id": "|".join(str(value) for value in second_execution["outcome_ids"]),
        }
    )
    emitted_second["transition_id"] = f"{episode_id}:decision1"
    ...
    evaluation_denominator["denominator_file_id"] = "denominator-test"
    payload: dict[str, object] = {
        "schema_version": "hm3d-exploration-execution-v1",
        "status": "EXPLORATION_EXECUTION_SMOKE_COMPLETE",
        "synthetic": False,
        "formal_result": False,
        ...
        "public_candidate_pool_id": "pool-0",
        **schema_fields,
        "public_contract_id": "public-contract-0",
        "evaluation_denominator_id": evaluation_denominator["denominator_file_id"],
        ...
        "execution_profile_id": "9" * 64,
        "cf2x_usd_id": "a" * 64,
        "sensor_profile_id": "b" * 64,
        "split_manifest_id": "6" * 64,
        "transit_time_model_id": "c" * 64,
        "communication_contract": _communication_contract(),
        "communication_contract_id": "exploration-test-intermittent",
        "engineering_debug": {
            "execution": {
                "communication": {
                    "telemetry_update_hz": 10.0,
        ...
        "execution": {
            ...
            "outcome_ids": execution["outcome_ids"] + ["3" * 64],
...
def _rewrite_record_id(payload: dict[str, object]) -> None:
    payload["runtime_record_id"] = "record-test"
...
def test_outcome_dataset_indexes_event_driven_reachable_component_records(tmp_path) -> None:
    ...
    payload = _raw()
    payload["split_manifest_id"] = split_manifest["split_manifest_id"]
    _rewrite_record_id(payload)
    record_path = tmp_path / "exploration_event_driven.json"
    ...
    assert row["evaluation_denominator_id"] == payload["evaluation_denominator_id"]
    assert (
        row["evaluation_geometry_denominator_id"]
        == payload["evaluation_geometry_denominator_id"]
    )
...
def test_outcome_dataset_rejects_legacy_completion_and_split_mismatch(tmp_path) -> None:
    ...
    payload["split_manifest_id"] = "0" * 64
    _rewrite_record_id(payload)
    record_path.write_text(json.dumps(payload), encoding="utf-8")
...
def test_outcome_dataset_rejects_mixed_execution_profiles(tmp_path) -> None:
    split_manifest = _split_manifest()
    first = _raw(episode_id="exploration-outcome-a")
    second = _raw(episode_id="exploration-outcome-b")
    for payload in (first, second):
        payload["split_manifest_id"] = split_manifest["split_manifest_id"]
        _rewrite_record_id(payload)
    second["execution_profile_id"] = "7" * 64
    _rewrite_record_id(second)
...
def _with_marvel_transition(payload: dict[str, object], *, action: int) -> dict[str, object]:
    ...
    for index, raw_decision in enumerate(decisions):
        ...
        transition: dict[str, object] = {
            ...
            "public_context_id": raw_decision["public_context_id"],
            "public_candidate_pool_id": raw_decision["public_candidate_pool_id"],
            **public_schema_fields(),
            ...
            "next_public_context_id": next_decision["public_context_id"],
            "next_public_candidate_pool_id": next_decision["public_candidate_pool_id"],
        ...
        transition["transition_id"] = f"marvel-transition-{index}"
    ...
    for index, raw_decision in enumerate(decisions):
        ...
        transition: dict[str, object] = {
            ...
            **public_schema_fields(),
            ...
            "execution_outcome_ids": execution["outcome_ids"],
            "outcome_id": "|".join(str(value) for value in execution["outcome_ids"]),
        }
        transition["transition_id"] = f"marl-ipp-transition-{index}"
        transitions.append(transition)
...
def test_training_ingestion_binds_each_real_decision_not_episode_aggregate() -> None:
    ...
    assert "manifest_id" not in aggregate
    samples = sample_from_exploration_training_record(
...
def test_training_ingestion_rejects_decision_schema_drift() -> None:
    ...
    with pytest.raises(ValueError, match="requires task_reservation_schema_version"):
        sample_from_exploration_training_record(
            payload,
            allowed_train_scene_ids=training_scene_ids_from_split_manifest(manifest),
        )
...
def test_training_ingestion_rejects_transition_schema_drift() -> None:
    ...
    first["transition_id"] = "transition-decision0-altered"
    _rewrite_record_id(payload)

    with pytest.raises(ValueError, match="requires candidate_pool_schema_version"):
        sample_from_exploration_training_record(
            payload,
            allowed_train_scene_ids=training_scene_ids_from_split_manifest(manifest),
        )
...
def test_marvel_supplementary_reference_rejects_current_and_legacy_transition_keys_together(
) -> None:
    manifest = _split_manifest()
...
def test_external_learning_ports_reject_planner_reward_substitution(
    ...
    transition["transition_id"] = "transition-altered"
    payload["runtime_record_id"] = "record-test"
...
def test_training_ingestion_rejects_mutated_reward_or_non_train_scene() -> None:
    ...
    transition["reward_explored_free_flight_volume_auc_time_contribution"] = 0.99
    transition["transition_id"] = "transition-altered"
    altered["runtime_record_id"] = "record-test"
    with pytest.raises(ValueError, match="actual decision AUC contribution"):
        sample_from_exploration_training_record(altered, allowed_train_scene_ids=allowed)
...
def test_training_performs_real_updates_and_records_only_train_rollout_provenance() -> None:
    ...
    checkpoint, provenance = train_single_rl_baseline(
        samples,
        split_manifest_id=str(manifest["split_manifest_id"]),
        updates=2,
        hidden_dim=8,
        seed=17,
