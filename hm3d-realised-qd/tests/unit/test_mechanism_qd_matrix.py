...
from realised_qd.evaluation.hm3d_mechanism_qd_matrix import (
    MechanismQDUnit,
...
def test_mechanism_qd_matrix_validates_only_a_paired_effect_with_admitted_mechanism() -> None:
    evidence = assemble_mechanism_qd_paired_evidence(_units())

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
...
def test_mechanism_qd_matrix_rejects_a_selector_that_never_changes_selection() -> None:
    evidence = assemble_mechanism_qd_paired_evidence(_units(selection_changed=False))

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert "QD_ALWAYS_ABSTAINED_FOR_UNCERTAIN_REALISATION" in evidence["reasons"]


def test_mechanism_qd_matrix_records_an_unfaithful_prediction_without_blocking(
) -> None:
    units = list(_units())
    for unit in units:
    ...
    evidence = assemble_mechanism_qd_paired_evidence(tuple(units))

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert evidence["reasons"] == []


def test_mechanism_qd_matrix_reports_when_qd_had_no_near_value_diverse_option() -> None:
    evidence = assemble_mechanism_qd_paired_evidence(_units(value_protected_opportunity=False))

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert "QD_VALUE_PROTECTED_DIVERSITY_OPPORTUNITY_INSUFFICIENT" in evidence["reasons"]


def test_mechanism_qd_matrix_rejects_non_positive_realised_qd_effect() -> None:
    evidence = assemble_mechanism_qd_paired_evidence(_units(realised_metric=0.20))

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert "REALISED_QD_HAS_NO_SIGNIFICANT_PAIRED_ADVANTAGE" in evidence["reasons"]


def test_mechanism_qd_matrix_rejects_a_statistically_positive_but_trivial_qd_effect() -> None:
    evidence = assemble_mechanism_qd_paired_evidence(_units(realised_metric=0.251))

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert "REALISED_QD_HAS_NO_SIGNIFICANT_PAIRED_ADVANTAGE" in evidence["reasons"]


def test_mechanism_qd_matrix_requires_a_train_only_descriptor_admission() -> None:
    units = list(_units())
    for unit in units:
    ...
    with pytest.raises(ValueError, match="richness evidence"):
        assemble_mechanism_qd_paired_evidence(tuple(units))


def test_mechanism_qd_matrix_keeps_train_replay_stability_as_a_diagnostic() -> None:
    units = list(_units())
    for unit in units:
        admission = unit.realised_qd["realised_qd"]["history"]["train_descriptor_admission"]
...
def test_mechanism_qd_matrix_keeps_train_mode_calibration_as_a_diagnostic() -> None:
    units = list(_units())
    for unit in units:
        admission = unit.realised_qd["realised_qd"]["history"]["train_descriptor_admission"]
...
def test_mechanism_qd_matrix_allows_a_diagnostic_calibration_to_vary() -> None:
    units = list(_units())
    for unit in units:
        admission = unit.realised_qd["realised_qd"]["history"]["train_descriptor_admission"]
...
def test_mechanism_qd_matrix_rejects_a_train_admission_without_frozen_split_provenance() -> None:
    units = list(_units())
    for unit in units:
        admission = unit.realised_qd["realised_qd"]["history"]["train_descriptor_admission"]
...
def test_mechanism_qd_matrix_rejects_active_qd_without_a_populated_outcome_archive() -> None:
    units = tuple(
        MechanismQDUnit(
            unit_id=f"unit{index}",
            no_qd=_record(strategy="no_qd", unit_index=index, metric=0.20),
    ...
    evidence = assemble_mechanism_qd_paired_evidence(units)

    assert evidence["status"] == "MECHANISM_QD_PILOT_COMPLETE"
    assert "QD_ACTIVE_WITHOUT_A_RICH_OUTCOME_GROUNDED_ARCHIVE" in evidence["reasons"]


def test_mechanism_qd_matrix_rejects_pairing_or_backbone_drift() -> None:
    ...
    with pytest.raises(ValueError, match="candidate-value backbone"):
        MechanismQDUnit("unit0", no_qd, planned_qd, realised_qd)
