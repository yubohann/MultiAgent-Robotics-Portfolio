"""Deterministic environment-side initial-state selection for HM3D Exploration."""

from __future__ import annotations

import numpy as np

EXPLORATION_START_RESET_SCHEMA_VERSION = "hm3d-start-reset-candidates-v1"
...
EXPLORATION_START_RESET_DEPARTURE_WITNESS_SCHEMA_VERSION = "hm3d-start-departure-witness-v1"


def _largest_component_grid(
    arrays: dict[str, object],
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, int]:
    """Validate Geometry arrays and return their largest retained component."""
    ...
    if component == 0:
        raise ValueError("Geometry ESDF has no retained free-flight component")
...
def largest_component_clearance_points(
    ...
    *,
    ...
    if not np.any(eligible):
        raise ValueError("largest Geometry component has no reset points at the mobility clearance")
...
def largest_component_departure_witnesses(
    ...
    *,
    ...
    if not len(start_indices):
        raise ValueError("largest Geometry component has no route-sample-clear first departure")
