"""Target-free joint candidates for the HM3D Exploration weak-baseline matrix."""
...
from realised_qd.runtime.hm3d_realised_qd import HM3D_CANDIDATE_INTENT_SPEC
from realised_qd.runtime.hm3d_team_collaboration import (
    audit_translation_invariant_team_trajectories,
)
from realised_qd.runtime.hm3d_trajectory import (
...
class PublicSearchState:
    """Strictly public input shared by every ranked Exploration method."""
    ...
    # Development default for isolated unit fixtures; real Exploration workers pass the
...
# Same opportunity definition as the Exploration outcome summary; a selection tie-break only.
...
def identity_path_guard(agent_id: str, path_m: tuple[Point3, ...]) -> GuardedPath:
    """Test-only pass-through guard; never use this function for Exploration."""
...
def build_public_candidate_pool(
    ...
    *,
    ...
    """Build the shared Exploration action authority before any baseline ranks it."""
...
def _candidate_semantic_id(manifest: CandidateFragmentManifest) -> str:
    ...
    fragment_ids = ",".join(
        fragment["agent_id"] + "-" + fragment["fragment_type"] for fragment in fragments
    )
    return (
