"""Fail-closed training ingestion for the ordinary Exploration single-RL baseline."""
...
from realised_qd.evaluation.hm3d_exploration_matrix import ExplorationProbeRecord
from realised_qd.learning.rb_sf_sac import RBSFSAC, RBSFSACConfig
from realised_qd.learning.replay import CandidateTransition

SINGLE_RL_TRAINING_PROVENANCE_SCHEMA_VERSION = "hm3d-single-rl-training-v2"
_CONTEXT_DIM = 4
_CANDIDATE_DIM = 5
...
def sample_from_exploration_training_record(
    ...
    require_current_public_schema(payload, context="single-RL Exploration worker record")
    probe = ExplorationProbeRecord.from_raw(str(payload.get("strategy")), payload)
    ...
    for index, (raw_emitted, raw_decision) in enumerate(
        ...
        if terminated is not expected_terminated or truncated is not expected_truncated:
            raise ValueError("training transition terminal cause differs from Exploration outcome")
...
def train_single_rl_baseline(
    ...
    *,
    ...
    if len(ordered) < minimum_transitions:
        raise ValueError("not enough real Exploration decision transitions for single-RL training")
    if len({sample.transition_id for sample in ordered}) != len(ordered):
        raise ValueError("duplicate Exploration decision transition supplied to single-RL training")
