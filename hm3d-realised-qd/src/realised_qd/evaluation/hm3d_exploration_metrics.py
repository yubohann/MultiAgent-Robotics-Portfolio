...
def evaluation_denominator_id(scene_rows: tuple[dict[str, object], ...]) -> str:
    """Return a readable denominator identity for a Geometry scene cohort."""
    ...
    if not scene_rows:
        raise ValueError("evaluation denominator requires at least one Geometry scene")
    manifest: list[dict[str, object]] = []
    seen: set[str] = set()
    for index, raw in enumerate(scene_rows):
        if not isinstance(raw, dict) or not required.issubset(raw):
            raise ValueError(f"Geometry denominator row[{index}] is incomplete")
