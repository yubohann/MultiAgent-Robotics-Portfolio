"""Fail-closed assembly for the Mechanism realised-QD paired comparison."""
...
MECHANISM_QD_PAIRED_EVIDENCE_SCHEMA_VERSION = "hm3d-qd-paired-evidence-v1"
...
def _verify_worker(payload: Mapping[str, Any], strategy: str) -> None:
    if payload.get("schema_version") != "hm3d-exploration-execution-v1":
        raise ValueError("Mechanism QD evidence needs current exploration worker records")
    if payload.get("status") != "EXPLORATION_EXECUTION_SMOKE_COMPLETE":
        raise ValueError("Mechanism QD evidence cannot score a failed Exploration worker")
    if payload.get("synthetic") is not False or payload.get("formal_result") is not False:
        raise ValueError(
            "Mechanism QD evidence must use non-synthetic, non-formal Exploration records"
        )
    if payload.get("exploration_task_validity_closed") is not False:
        raise ValueError("a worker may not claim Exploration closure")
    if payload.get("selection_partition") != "validation":
        raise ValueError("Mechanism QD comparison may use validation records only")
    if payload.get("strategy") != strategy:
        raise ValueError("Mechanism QD input strategy does not match its declared branch")
    require_current_public_schema(payload, context="Mechanism QD worker record")
    _require_id(payload.get("runtime_record_id"), "Exploration runtime record id")
    for name in _PAIR_FIELDS:
        if name not in payload:
            raise ValueError(f"Exploration record lacks pairing field: {name}")
    if not isinstance(payload["scene_id"], str) or not payload["scene_id"]:
        raise ValueError("Exploration scene_id is invalid")
    if payload["fleet_size"] != FORMAL_FLEET_SIZE:
        raise ValueError(f"Mechanism QD comparison requires N={FORMAL_FLEET_SIZE}")
    if not isinstance(payload["random_key"], int) or isinstance(payload["random_key"], bool):
        raise ValueError("Exploration random_key is invalid")
    for name in (
        "public_context_id",
        "public_candidate_pool_id",
        "sensor_profile_id",
    ...
    if not isinstance(metric, Mapping):
        raise ValueError("Exploration worker lacks exploration metric report")
    auc = _number(metric.get("explored_free_flight_volume_auc_time"), "exploration AUC")
    if not 0.0 <= auc <= 1.0:
        raise ValueError("exploration AUC must lie in [0, 1]")


@dataclass(frozen=True, slots=True)
class MechanismQDUnit:
    ...
    def __post_init__(self) -> None:
        require_identifier(self.unit_id, "Mechanism QD unit_id")
        records = {
            "no_qd": self.no_qd,
        ...
        for strategy, payload in records.items():
            if not isinstance(payload, Mapping):
                raise TypeError(f"Mechanism QD {strategy} record must be an object")
        ...
        for strategy, payload in tuple(records.items())[1:]:
            ...
            if drift:
                raise ValueError(f"Mechanism QD unit has pair drift in {strategy}: {drift}")
            if payload["selector_backbone_id"] != anchor["selector_backbone_id"]:
                raise ValueError("Mechanism QD controls must share one candidate-value backbone")
...
def _selection_records(payload: Mapping[str, Any]) -> tuple[Mapping[str, Any], ...]:
    ...
    if not isinstance(decisions, list) or not decisions:
        raise ValueError("Mechanism QD record has no online decision trace")
    rows: list[Mapping[str, Any]] = []
    for decision in decisions:
        if not isinstance(decision, Mapping):
            raise ValueError("Mechanism QD decision trace is malformed")
        selection = decision.get("selection")
        if not isinstance(selection, Mapping):
            raise ValueError("Mechanism QD decision lacks selector audit")
...
def _candidate_admissions(
    ...
    for payload in payloads:
        ...
        if not isinstance(qd, Mapping):
            raise ValueError("Mechanism QD record lacks realised-QD diagnostics")
        audits = qd.get("candidate_intent_audits")
        if not isinstance(audits, list) or not audits:
            raise ValueError("Mechanism QD record lacks candidate-intent audits")
        for audit in audits:
    ...
    if (
        ...
        raise ValueError("Mechanism QD candidate admission thresholds drifted across workers")
    return (
        assessed,
        admitted,
...
def _value_protected_candidate_opportunities(
    ...
    for payload in payloads:
        ...
        if not isinstance(audits, list) or not audits:
            raise ValueError("Mechanism QD record lacks value-protected diversity audits")
        for audit in audits:
            if not isinstance(audit, Mapping):
    ...
    if len(minimum_candidates) != 1 or len(minimum_joint_cells) != 1 or len(utility_slacks) != 1:
        raise ValueError(
            "Mechanism QD value-protected opportunity thresholds drifted across workers"
        )
    return (
        assessed,
        admitted,
...
def _realised_outcomes(
    ...
    for payload in payloads:
        ...
        if not isinstance(qd, Mapping) or not isinstance(qd.get("admissions"), list):
            raise ValueError("Mechanism QD record lacks outcome admissions")
        for entry in qd["admissions"]:
            ...
            if (
                ...
                raise ValueError("Mechanism QD outcome admission is malformed")
            if entry.get("executed") is not True:
                raise ValueError("Mechanism QD admission does not describe an executed candidate")
            if not isinstance(entry.get("candidate_id"), str) or not entry["candidate_id"]:
                raise ValueError("Mechanism QD admission lacks its executed candidate ID")
            _require_id(entry.get("execution_outcome_id"), "Mechanism QD execution outcome id")
            raw_footprint = entry.get("public_new_free_voxel_keys")
            if not isinstance(raw_footprint, list) or not raw_footprint:
                raise ValueError("Mechanism QD admission lacks its public execution footprint")
            ...
            for key_index, raw_key in enumerate(raw_footprint):
                if (
                    ...
                    raise ValueError(
                        f"Mechanism QD public execution footprint key {key_index} is invalid"
                    )
                footprint.append((raw_key[0], raw_key[1], raw_key[2]))
            intents.append(
                tuple(_number(value, "Mechanism QD candidate intent") for value in intent)
            )
            descriptors.append(
                RealisedQDDescriptor(
...
def _verified_train_descriptor_admission(
    ...
    """Verify the compact train-only QD admission used by every Mechanism worker."""

    admissions: list[Mapping[str, Any]] = []
    for payload in payloads:
        qd = payload.get("realised_qd")
        if not isinstance(qd, Mapping):
            raise ValueError("Mechanism realised-QD record lacks QD diagnostics")
        history = qd.get("history")
        if not isinstance(history, Mapping):
            raise ValueError("Mechanism realised-QD record lacks train-history admission")
        admission = history.get("train_descriptor_admission")
        if not isinstance(admission, Mapping):
            raise ValueError("Mechanism realised-QD record lacks train descriptor admission")
        _require_id(
        ...
        if admission.get("status") != "QD_TRAIN_DESCRIPTOR_ADMITTED":
            raise ValueError("Mechanism realised-QD record uses a non-admitted train descriptor")
        if admission.get("descriptor_schema_version") != HM3D_REALISED_QD_SCHEMA_VERSION:
            raise ValueError("Mechanism train descriptor schema does not match the frozen runtime")
        if admission.get("archive_spec_id") != HM3D_REALISED_QD_ARCHIVE_SPEC.spec_id:
            raise ValueError("Mechanism train archive spec does not match the frozen runtime")
        ...
        if (
            ...
            raise ValueError("Mechanism train descriptor admission has too few outcomes")
        if not isinstance(scene_ids, list) or len(set(scene_ids)) < 2:
            raise ValueError("Mechanism train descriptor admission lacks cross-scene evidence")
        _require_id(split_manifest_id, "Mechanism train descriptor split manifest id")
        if not isinstance(source_ids, list) or not source_ids:
            raise ValueError("Mechanism train descriptor admission lacks source record ids")
        for value in source_ids:
            _require_id(value, "Mechanism train descriptor source record id")
        richness = admission["richness_audit"]
        if not isinstance(richness, Mapping) or richness.get("status") != "QD_DESCRIPTOR_ADMITTED":
            raise ValueError("Mechanism train descriptor lacks realised-QD richness evidence")
        for name, minimum in (
            ...
            if not isinstance(value, int) or isinstance(value, bool) or value < minimum:
                raise ValueError(f"Mechanism train descriptor richness has invalid {name}")
        observed_effective_cells = _number(
            richness.get("joint_shannon_effective_cells"),
            "Mechanism train descriptor Shannon-effective cells",
        )
        declared_effective_floor = _number(
            richness.get("minimum_joint_shannon_effective_cells"),
            "Mechanism train descriptor Shannon-effective-cell floor",
        )
        if (
            ...
            raise ValueError("Mechanism train descriptor archive is not effectively populated")
        ...
        if (
            ...
            raise ValueError("Mechanism train descriptor axes are effectively collinear")
        ...
        if (
            ...
            raise ValueError("Mechanism train descriptor lacks execution-footprint separation")
        admissions.append(admission)
    ids = {str(admission["train_descriptor_admission_id"]) for admission in admissions}
    if len(ids) != 1:
        raise ValueError(
            "Mechanism realised-QD workers do not share one train descriptor admission"
        )
    return admissions[0]
...
def assemble_mechanism_qd_paired_evidence(units: Sequence[MechanismQDUnit]) -> dict[str, object]:
    """Assemble immutable mechanism diagnostics from paired workers."""

    rows = tuple(units)
    if len(rows) < 12:
        raise ValueError("Mechanism QD needs at least twelve paired validation units")
    # Revalidate here; evidence mappings are mutable, so a post-construction edit
    # could bypass the id, pairing or schema checks.
    for unit in rows:
        MechanismQDUnit(unit.unit_id, unit.no_qd, unit.planned_qd, unit.realised_qd)
    unit_ids = [row.unit_id for row in rows]
    if len(set(unit_ids)) != len(unit_ids):
        raise ValueError("Mechanism QD unit IDs must be unique")
    ...
    if len(scene_ids) < 2 or fleet_size_values != {FORMAL_FLEET_SIZE} or len(seeds) < 2:
        raise ValueError(f"Mechanism QD needs two scenes, N={FORMAL_FLEET_SIZE}, and two seeds")
    backbone_ids = {row.backbone_id for row in rows}
    if len(backbone_ids) != 1:
        raise ValueError("Mechanism QD controls do not share one candidate-value backbone")
    ...
    for selection in selection_rows:
        if not isinstance(selection.get("qd_abstained"), bool):
            raise ValueError("Mechanism QD selector audit lacks qd_abstained")
        if not isinstance(selection.get("archive_entry_count"), int):
            raise ValueError("Mechanism QD selector audit lacks archive_entry_count")
        if not isinstance(selection.get("archive_revision"), int):
            raise ValueError("Mechanism QD selector audit lacks archive_revision")
    ...
    return {
        "schema_version": MECHANISM_QD_PAIRED_EVIDENCE_SCHEMA_VERSION,
        "status": "MECHANISM_QD_PILOT_COMPLETE",
        "synthetic": False,
        "formal_result": False,
        "claim_limit": (
            "QD-only Mechanism diagnostic. It does not validate OGFR, RB-SF-SAC, "
            "Mechanism as a whole, or P10."
        ),
...
__all__ = [
    "MECHANISM_QD_PAIRED_EVIDENCE_SCHEMA_VERSION",
    "MINIMUM_QD_ARCHIVE_ENTRIES_FOR_ACTIVE_SELECTION",
    ...
    "MechanismQDUnit",
    "assemble_mechanism_qd_paired_evidence",
]
