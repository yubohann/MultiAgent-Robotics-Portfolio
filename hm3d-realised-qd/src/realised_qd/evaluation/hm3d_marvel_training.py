...
def sample_from_exploration_training_record(
    ...
    require_current_public_schema(payload, context="MARVEL Exploration worker record")
    strategy = str(payload.get("strategy"))
    probe = ExplorationProbeRecord.from_raw(strategy, payload)
    ...
    for index, (raw_emitted, raw_decision) in enumerate(
        ...
        if terminated is not expected_terminated or truncated is not expected_truncated:
            raise ValueError(
                "MARVEL supplementary reference terminal cause differs from Exploration outcome"
            )
