...
from realised_qd.contracts.io import finite_number, require_identifier, to_primitive

ABI_VERSION = "realised-qd-hm3d-exploration-v1"
...
def _numeric_pairs(values: Any, name: str) -> NumericPairs:
    pairs = [
        (require_identifier(raw_key, f"{name} key"), finite_number(raw_value, f"{name}.{raw_key}"))
        for raw_key, raw_value in tuple(values)
    ]
    return tuple(sorted(pairs, key=lambda item: item[0]))
...
class FragmentInstance:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("source_observation_id", "sender_id", "receiver_id", "message_id"):
            value = getattr(self, name)
            if value is not None:
                require_identifier(value, name)
        fragment_type = self.type_signature.fragment_type
        # Each fragment type has a fixed geometric shape that execution relies on.
        ...
        if fragment_type == "communication":
            if self.sender_id is None or self.receiver_id is None or self.message_id is None:
                raise ValueError("communication fragment requires sender, receiver and message ID")
            if self.sender_id == self.receiver_id:
                raise ValueError("communication sender and receiver must differ")
        elif any(
            value is not None for value in (self.sender_id, self.receiver_id, self.message_id)
        ):
    ...
    def fragment_id(self) -> str:
        return self.instance_fragment_id
...
class CandidateFragmentManifest:
    ...
    def fragment_ids(self) -> tuple[str, ...]:
        return tuple(fragment.instance_fragment_id for fragment in self.fragments)
...
class InteractionEdge:
    ...
    def __post_init__(self) -> None:
        _schema(self.schema_version)
        require_identifier(self.source_fragment_id, "source_fragment_id")
        require_identifier(self.target_fragment_id, "target_fragment_id")
        ...
        # Store edges in a canonical direction so equal edges compare equal.
        if self.source_fragment_id > self.target_fragment_id:
            source = self.target_fragment_id
            target = self.source_fragment_id
            object.__setattr__(self, "source_fragment_id", source)
            object.__setattr__(self, "target_fragment_id", target)
...
class CandidateGraphBatch:
    ...
    def __post_init__(self) -> None:
        ...
        fragments = tuple(sorted(set(self.fragment_ids)))
        ...
        for candidate_id, fragment_id, order in edges:
            if candidate_id not in candidates or fragment_id not in fragments:
        ...
        object.__setattr__(self, "fragment_ids", fragments)
...
class ActionToken:
    ...
    context_id: str
    manifest_id: str
    planned_fragment_ids: tuple[str, ...]
    issued_at: float
    duration: float
    schema_version: str = ABI_VERSION
    ...
    def __post_init__(self) -> None:
        _schema(self.schema_version)
        for name in ("token_id", "episode_id", "decision_id", "context_id", "manifest_id"):
            require_identifier(getattr(self, name), name)
        fragment_ids = tuple(self.planned_fragment_ids)
        if not fragment_ids:
            raise ValueError("ActionToken requires planned fragment IDs")
        object.__setattr__(self, "planned_fragment_ids", fragment_ids)
        ...
        return to_primitive(
            {
                ...
                "planned_fragment_ids": self.planned_fragment_ids,
...
class FragmentOutcome:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "decision_id": self.decision_id,
            "agent_id": self.agent_id,
            "planned_fragment_id": self.planned_fragment_id,
            "executed": self.executed,
...
class FragmentReplayRecord:
    ...
    def __post_init__(self) -> None:
        _schema(self.schema_version)
        for name in ("instance_fragment_id", "fragment_type", "outcome_id", "context_id"):
            require_identifier(getattr(self, name), name)
    ...
    def from_dict(cls, payload: dict[str, Any]) -> FragmentReplayRecord:
        return cls(
            ...
            fragment_type=payload["fragment_type"],
...
class ReplayDecision:
    ...
    def __post_init__(self) -> None:
        ...
        if self.allowed and self.reason_code != "ALLOW":
            raise ValueError("allowed replay decision must use ALLOW reason")
        if not self.allowed and self.reason_code == "ALLOW":
            raise ValueError("denied replay decision cannot use ALLOW reason")
