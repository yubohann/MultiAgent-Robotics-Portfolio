...
from realised_qd.contracts import FORMAL_FLEET_SIZE  # noqa: E402
from realised_qd.runtime.hm3d_realised_qd import (  # noqa: E402
...
def _record_manifests(payload: dict[str, Any]) -> tuple[str, ...]:
    if payload.get("schema_version") != "hm3d-exploration-execution-v1":
        raise ValueError("QD replay calibration output is not a current Exploration worker record")
...
def run_calibration(
    ...
    return {
        ...
        "claim_limit": (
            ...
            "it establishes neither a QD task gain nor a Mechanism or Formal result."
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--runner",
        type=Path,
        default=ROOT / "scripts" / "exploration" / "run_exploration_episode.py",
    )
