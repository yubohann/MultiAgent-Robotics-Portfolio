...
from realised_qd.runtime.hm3d_cf2x_vectorized_execution import (
    IsaacCF2XVectorizedExecutionBackend,
)
from realised_qd.runtime.hm3d_multicluster import (
    HM3DClusterLayout,
    cluster_seed,
...
def main(args: argparse.Namespace, simulation_app: Any) -> int:
    ...
    UsdGeom.Xform.Define(stage, f"{env_paths[0]}/ExplorationAgents")
    for agent_index, position in enumerate(starts):
        agent = UsdGeom.Xform.Define(stage, f"{env_paths[0]}/ExplorationAgents/Agent_{agent_index}")
    ...
    robot = Multirotor(
        cf2x._multirotor_cfg(cf2x_usd, args.physics_dt_s).replace(
            prim_path="/World/envs/env_.*/ExplorationAgents/Agent_.*/Robot"
        )
    )
