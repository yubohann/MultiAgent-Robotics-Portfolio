...
from realised_qd.evaluation.hm3d_exploration_contract import (  # noqa: E402
    DEFAULT_PATH as DEFAULT_EXPLORATION_CONTRACT,
)
from realised_qd.evaluation.hm3d_exploration_contract import (  # noqa: E402
    load_exploration_observation_contract,
)
from realised_qd.evaluation.hm3d_exploration_metrics import (  # noqa: E402
    evaluation_denominator_id,
)
from realised_qd.runtime.hm3d_belief import (  # noqa: E402
    PublicRangeRayOutcome,
    SparseVoxelBelief,
)
from realised_qd.runtime.range_sensing import (  # noqa: E402
...
def _cohort_rows(geometry_artifact: dict[str, Any]) -> tuple[dict[str, object], ...]:
    payload = geometry_artifact.get("payload")
    if not isinstance(payload, dict) or geometry_artifact.get("phase_id") != "Geometry":
        raise ValueError("--geometry-artifact must be a Geometry preflight artifact")
    rows = payload.get("scenes")
    if not isinstance(rows, list) or not rows or not all(isinstance(row, dict) for row in rows):
        raise ValueError("Geometry artifact needs non-empty scene rows")
    return tuple(rows)


def _matching_geometry_row(rows: tuple[dict[str, object], ...], scene_id: str) -> dict[str, object]:
    matches = [row for row in rows if row.get("scene_id") == scene_id]
    if len(matches) != 1:
        raise ValueError("Geometry artifact must contain this scene exactly once")
    return matches[0]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--geometry-artifact", type=Path, required=True)
...
def main(args: argparse.Namespace, simulation_app: Any) -> int:
    ...
    for name in ("collision", "positions", "flight", "geometry", "contract"):
    ...
    geometry_rows = _cohort_rows(_read_object(paths["geometry"]))
    geometry_row = _matching_geometry_row(geometry_rows, args.scene_id)
    ...
    expected_collision_sha = str(geometry_row["collision_geometry_id"])
    if _file_id(paths["collision"]) != expected_collision_sha:
        raise ValueError("collision USD id differs from frozen Geometry flight space")
    if flight.get("source_glb_file_id") != geometry_row["source_geometry_id"]:
        raise ValueError("flight-space source geometry differs from frozen Geometry")
    if flight.get("flight_space_manifest_id") != geometry_row["flight_space_manifest_id"]:
        raise ValueError("flight-space manifest differs from frozen Geometry")
    ...
    denominator_digest = evaluation_denominator_id(geometry_rows)
    ...
    payload = _public_payload(
        scene_id=args.scene_id,
        source_geometry_id=str(geometry_row["source_geometry_id"]),
        collision_usd_file_id=expected_collision_sha,
        flight_space_manifest_id=str(geometry_row["flight_space_manifest_id"]),
