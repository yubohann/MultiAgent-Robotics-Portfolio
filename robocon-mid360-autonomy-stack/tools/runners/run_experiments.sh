...
world="${WORLD:-$workspace/install/mid360_simulation/share/mid360_simulation/worlds/indoor_competition_candidate.world}"
...
lio_script="$workspace/src/mid360_simulation/scripts/shell/run_controlled_lio_smoke.sh"
mapping_script="$workspace/src/mid360_simulation/scripts/shell/run_simulation_mapping.sh"
localization_script="$workspace/src/mid360_simulation/scripts/shell/run_fixed_map_localization_smoke.sh"
interlock_script="$workspace/src/mid360_simulation/scripts/shell/run_localization_recovery_interlock_smoke.sh"
synthetic_script="$workspace/src/game_supervisor/tools/run_synthetic_runtime_smoke.sh"
fault_script="$workspace/src/game_supervisor/tools/run_competition_fault_runtime_smoke.sh"
ab_script="$workspace/tools/runners/run_pointlio_ab_smoke.sh"
rgbd_script="$workspace/tools/runners/run_rgbd_quality_smoke.sh"
rviz_config="$workspace/src/mid360_simulation/config/gazebo_mapping.rviz"

usage() {
  cat <<'EOF'
用法：
  bash tools/runners/run_experiments.sh all          顺序运行全部有界自动实验
  bash tools/runners/run_experiments.sh quality      LIO + 建图 + 固定地图定位
  bash tools/runners/run_experiments.sh faults       失锁互锁 + 比赛总控 + 双车/机构故障
  bash tools/runners/run_experiments.sh lio          只运行 30000-ray LIO 受控实验
  bash tools/runners/run_experiments.sh mapping      只运行建图并生成 PCD
  bash tools/runners/run_experiments.sh localization 只运行固定地图定位
  bash tools/runners/run_experiments.sh interlock    只运行失锁安全互锁
  bash tools/runners/run_experiments.sh competition 只运行总控和两类故障
  bash tools/runners/run_experiments.sh ab          只运行 FAST-LIO2 / Point-LIO 同包回放
  bash tools/runners/run_experiments.sh rgbd        只运行 Gazebo RGB-D 质量探针
  bash tools/runners/run_experiments.sh gui         启动可见 Gazebo + RViz（按 Ctrl+C 停止）
  bash tools/runners/run_experiments.sh --dry-run all 只打印 all 将执行的命令
...
run_gui() {
  ...
  printf 'GUI 运行目录：%s\n' "$dir"
  setsid ros2 launch mid360_simulation gazebo_mid360_mapping.launch.py \
