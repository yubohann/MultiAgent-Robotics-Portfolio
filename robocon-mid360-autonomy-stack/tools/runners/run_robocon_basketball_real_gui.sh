...
export GAZEBO_MODEL_PATH="$WORKSPACE/install/mid360_simulation/share/mid360_simulation/models${GAZEBO_MODEL_PATH:+:$GAZEBO_MODEL_PATH}"
mkdir -p /tmp/xk
...
setsid ros2 launch mid360_simulation gazebo_mid360_dual_mapping.launch.py \
...
start_gui "$((DISPLAY_BASE + 1))" "$RUN_DIR/rviz_pointcloud.log" rviz2 -d "$WORKSPACE/src/mid360_simulation/config/gazebo_mapping.rviz"
start_gui "$((DISPLAY_BASE + 2))" "$RUN_DIR/rviz_map2d.log" rviz2 -d "$WORKSPACE/src/mid360_simulation/config/gazebo_2d_mapping.rviz"
start_gui "$((DISPLAY_BASE + 3))" "$RUN_DIR/rviz_rgbd.log" rviz2 -d "$WORKSPACE/src/mid360_simulation/config/rviz_basketball_rgbd.rviz"

capture_window() {
  local display_num=$1 title=$2 output_dir=$3 prefix=$4
  setsid timeout --signal=INT "$((DURATION + 90))" python3 "$WORKSPACE/tools/capture/capture_xvfb_frames.py" \
...
timeout --signal=INT "$((DURATION + 120))" python3 \
  "$WORKSPACE/install/mid360_simulation/lib/mid360_simulation/basketball_demo_controller.py" \
