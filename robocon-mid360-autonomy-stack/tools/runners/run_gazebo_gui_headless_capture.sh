...
setsid ros2 launch mid360_simulation gazebo_mid360_lio.launch.py \
  ...
  world:="$workspace/install/mid360_simulation/share/mid360_simulation/worlds/indoor_competition_candidate.world" > "$run_dir/gazebo_lio.log" 2>&1 &
lio_pid=$!
gzclient_pid=""
motion_pid=""
cleanup() {
...
timeout 90s python3 "$workspace/tools/capture/capture_xvfb_frames.py" \
