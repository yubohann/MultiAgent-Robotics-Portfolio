...
sys.path.insert(0, str(PACKAGE_ROOT / "scripts" / "python"))
...
class SimulationAssetTests(unittest.TestCase):
    ...
    def test_sparse_coverage_mapping_is_explicitly_diagnostic(self):
        ...
        runner = (PACKAGE_ROOT / "scripts" / "shell" / "run_simulation_mapping.sh").read_text(encoding="utf-8")
    ...
    def test_one_command_dispatcher_does_not_mix_failed_mapping_with_archived_map(self):
        dispatcher = (PACKAGE_ROOT.parent.parent / "tools" / "runners" / "run_experiments.sh").read_text(encoding="utf-8")
    ...
    def test_fixed_map_interlock_publishes_expiring_peer_heartbeat(self):
        driver = (PACKAGE_ROOT / "scripts" / "python" / "fixed_map_interlock_integration.py").read_text(encoding="utf-8")
    ...
    def test_fixed_map_smoke_keeps_map_provenance_and_isolation(self):
        runner = (PACKAGE_ROOT / "scripts" / "shell" / "run_fixed_map_localization_smoke.sh").read_text(encoding="utf-8")
        probe = (PACKAGE_ROOT / "scripts" / "python" / "fixed_map_localization_smoke.py").read_text(encoding="utf-8")
    ...
    def test_plugin_caches_pattern_directions_and_skips_unused_cloud_mirror(self):
        source = (PACKAGE_ROOT / "src" / "livox_points_plugin.cpp").read_text(encoding="utf-8")
        header = (PACKAGE_ROOT / "include" / "mid360_simulation" / "livox_points_plugin.h").read_text(
            encoding="utf-8"
        )
    ...
    def test_stream_probe_requires_non_overlapping_livox_packets(self):
        probe = (PACKAGE_ROOT / "scripts" / "python" / "probe_simulation_streams.py").read_text(encoding="utf-8")
    ...
    def test_rgbd_quality_and_systemd_harnesses_are_bounded(self):
        probe = (PACKAGE_ROOT.parent.parent / "tools" / "metrics" / "rgbd_quality_probe.py").read_text(encoding="utf-8")
        rgbd_runner = (PACKAGE_ROOT.parent.parent / "tools" / "runners" / "run_rgbd_quality_smoke.sh").read_text(encoding="utf-8")
        systemd_runner = (PACKAGE_ROOT.parent.parent / "tools" / "runners" / "test_systemd_simulation.sh").read_text(encoding="utf-8")
