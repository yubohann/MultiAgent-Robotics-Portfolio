...
mapping_params="$workspace/install/mid360_simulation/share/mid360_simulation/config/fast_lio_mapping_simulation.yaml"
world_file="$workspace/install/mid360_simulation/share/mid360_simulation/worlds/indoor_competition_candidate.world"
mkdir -p "$run_dir"
...
if pgrep -f 'gzserver .*mid360_simulation' >/dev/null 2>&1; then
  echo "fixed_map_smoke_failed: an existing Gazebo server would contaminate this run" > "$run_dir/failure_reason.txt"
  exit 1
fi
export ROS_DOMAIN_ID="$ros_domain_id"

setsid ros2 launch mid360_simulation gazebo_mid360_lio.launch.py \
...
set +e
timeout 240s python3 "$workspace/src/mid360_simulation/scripts/python/fixed_map_localization_smoke.py" \
