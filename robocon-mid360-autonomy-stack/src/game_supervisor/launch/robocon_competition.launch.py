...
def generate_launch_description() -> LaunchDescription:
    ...
    supervisor_share = get_package_share_directory("game_supervisor")
    camera_share = get_package_share_directory("camera_yolo_adapter")
    map_localizer_share = get_package_share_directory("mid360_map_localizer")
    perception_share = get_package_share_directory("perception_adapter")
    ...
    supervisor = Node(
        package="game_supervisor",
        executable="game_supervisor",
        name="game_supervisor",
        output="screen",
        parameters=[supervisor_params],
    )
    pose_bridge = Node(
        package="pose_command_bridge",
        executable="pose_command_bridge",
        name="pose_command_bridge",
        output="screen",
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            default_value=get_package_share_directory("pose_command_bridge") + "/config/pose_command_bridge.yaml",
        ),
        DeclareLaunchArgument(
