...
setsid ros2 run game_supervisor game_supervisor --ros-args \
...
if [[ "$mode" == "mechanism" ]]; then
  setsid ros2 run game_supervisor robocon_action_simulator --ros-args \
    -p task_id:=competition-fault-smoke -p failure_action:=ExecutePass > "$run_dir/action_simulator.log" 2>&1 &
  action_pid=$!
  setsid ros2 run perception_adapter target_gate > "$run_dir/target_gate.log" 2>&1 &
  target_pid=$!
fi
...
set +e
timeout 35s python3 "$workspace/src/game_supervisor/tools/competition_fault_runtime_smoke.py" \
