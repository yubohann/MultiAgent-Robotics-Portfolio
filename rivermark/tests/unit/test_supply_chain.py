    ...
    if cleared:
        for index, asset in enumerate(payload["assets"]):
            asset["decision_record"] = {
                ...
                "evidence_identity": str(index + 1) * 16,
            }
    return payload
...
def _write_verified_release_artifacts(root: Path, payload: dict) -> None:
    ...
    payload["sbom"].update({"identity": _identity(sbom_path)})
    payload["signature"] = {
        ...
        "public_key_identity": _identity(public_key_path),
    }
    signature_path.write_bytes(private_key.sign(canonical_supply_chain_bytes(payload)))
    payload["signature"]["identity"] = _identity(signature_path)
    payload["signature"]["manifest_identity"] = supply_chain_identity(payload)
...
class SupplyChainTests(unittest.TestCase):
    ...
    def test_private_uri_and_unknown_field_strict(self) -> None:
        payload = _manifest()
    ...
    def test_verified_release_binds_signature_to_canonical_manifest(self) -> None:
        ...
        payload["signature"] = {
            ...
            "public_key_path": "artifacts/release.ed25519.pub",
            "public_key_uri": "https://example.org/rivermark/release.ed25519.pub",
            "public_key_identity": "e" * 16,
        }
        payload["signature"]["manifest_identity"] = supply_chain_identity(payload)
        self.assertEqual(validate_supply_chain_manifest(payload, require_release=True), ())
        altered = copy.deepcopy(payload)
        altered["assets"][0]["identity"] = "e" * 16
        self.assertIn(
            "signature_manifest_mismatch",
            {issue.code for issue in validate_supply_chain_manifest(altered, require_release=True)},
        )

    def test_file_verifier_reports_identity_and_status(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(report["manifest_identity"], supply_chain_identity(payload))
            self.assertEqual(report["asset_count"], 2)
            self.assertEqual(report["release_id"], "rivermark-dev-audit")

    def test_release_verifier_reads_identity_bound_sbom_and_ed25519_signature(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            payload = _manifest(cleared=True)
    ...
    def test_release_verifier_rejects_altered_sbom_and_wrong_public_key(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            payload = _manifest(cleared=True)
            _write_verified_release_artifacts(root, payload)
            ...
            report = verify_supply_chain_manifest(manifest_path, require_release=True)
            self.assertIn("sbom_identity", {issue["code"] for issue in report["issues"]})
            ...
            report = verify_supply_chain_manifest(manifest_path, require_release=True)
            self.assertIn("signature_public_key_identity", {issue["code"] for issue in report["issues"]})
