...
from rivermark_benchmark.runtime.preflight import (
...
class PreflightTests(unittest.TestCase):
    ...
    def test_runtime_gpu_probe_parses_and_enforces_vram_and_driver(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            with patch("rivermark_benchmark.runtime.preflight._probe_nvidia_smi", return_value=([gpu], None)):
                report = run_preflight(
    ...
    def test_nvidia_smi_probe_is_non_shell_and_bounded(self) -> None:
        ...
        with (
            patch("rivermark_benchmark.runtime.preflight.shutil.which", return_value="nvidia-smi.exe") as which,
            patch("rivermark_benchmark.runtime.preflight.subprocess.run", return_value=completed) as run,
        ):
    ...
    def test_scene_contract_file_and_authority_are_checked_before_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("rivermark_benchmark.runtime.preflight.resolve_city_lite_authority", return_value=authority):
                report = run_preflight(
                    output_dir=root / "runs",
                    minimum_free_bytes=0,
                    runtime=RuntimePreflightRequirements(
    ...
    def test_scene_contract_identity_mismatch_fails_closed_without_resolving(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            contract.write_text("{}\n", encoding="utf-8")
            with patch("rivermark_benchmark.runtime.preflight.resolve_city_lite_authority") as resolver:
                report = run_preflight(
                    output_dir=Path(temporary),
                    minimum_free_bytes=0,
                    runtime=RuntimePreflightRequirements(
    ...
    def test_runtime_lock_is_checked_inside_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("rivermark_benchmark.runtime.preflight.resolve_city_lite_authority", return_value=authority),
                patch(
                    "rivermark_benchmark.runtime.runtime_lock.audit_runtime_lock",
