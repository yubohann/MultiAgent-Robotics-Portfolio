...
from rivermark_benchmark.audit.formal_dataset import (
    CONTENT_IDENTITY_INDEX_SCHEMA,
...
from rivermark_benchmark.audit.release_manifest import (
    ReleaseBuildError,
    build_release_manifest,
    validate_release_manifest,
)
from rivermark_benchmark.audit.supply_chain import (
    canonical_supply_chain_bytes,
    supply_chain_identity,
)
from rivermark_benchmark.data.abi import (
    OBSERVATION_ABI_SCHEMA,
    observation_abi_identity,
)
from rivermark_benchmark.runtime.failure_ledger import (
    load_failure_ledger,
...
def _write_text(path: Path, value: str) -> str:
    ...
    return identity_file(path)


def _release_supply_chain(data_receipt_identities: tuple[str, ...] = ()) -> dict[str, object]:
        ...
        assets.append(
            {
                ...
                "decision_record": {
                    ...
                    "approved_by": "Test release approver",
                    "approved_at": "2026-07-24T00:00:00Z",
                    "evidence_identity": IdentityAccumulator(
...
def _write_release_supply_chain(
    root: Path,
    *,
    data_receipt_identities: tuple[str, ...] = (),
) -> Path:
    from cryptography.hazmat.primitives import serialization
    from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey

    payload = _release_supply_chain(data_receipt_identities)
    ...
    sbom["identity"] = identity_file(sbom_path)
    ...
    signature["public_key_identity"] = identity_file(public_key_path)
    signature_path.write_bytes(private_key.sign(canonical_supply_chain_bytes(payload)))
    signature["identity"] = identity_file(signature_path)
    signature["manifest_identity"] = supply_chain_identity(payload)
...
def _candidate(
    ...
    *,
    ...
    scene_identity = _write_text(episode_root / "scenes" / "scene.json", '{"public":true}\n')
    task_identity = _write_text(episode_root / "tasks" / "task.json", '{"task":"search"}\n')
    state_identity = _write_text(episode_root / "streams" / "state.jsonl", '{"sim_time_ns":0}\n')
    action_identity = _write_text(episode_root / "streams" / "actions.jsonl", '{"sim_time_ns":0}\n')
    mission_identity = _write_text(episode_root / "streams" / "mission.json", '{"remaining_budget_ns":1}\n')
    message_identity = _write_text(episode_root / "streams" / "messages.jsonl", '{"sim_time_ns":0}\n')
            ...
            rgb_files.append(
                {
                    "agent_id": agent_id,
                    "path": relative,
                    "identity": _write_text(episode_root / relative, f"agent-{agent_id}\n"),
                }
            )
        index_path = episode_root / "cameras" / "rgb_identities.json"
        _write_json(
            index_path,
            {
                "schema": CONTENT_IDENTITY_INDEX_SCHEMA,
                "stream_id": "rgb",
                "files": rgb_files,
            },
        )
        streams.append(
            {
                ...
                "content_identity_index_path": "cameras/rgb_identities.json",
                "content_identity_index_identity": identity_file(index_path),
            }
        )
    ...
    manifest = {
        ...
        "layout": {
            ...
            "layout_identity": _digest("layout-a"),
            "layout_lineage_identity": layout_lineage,
        ...
        "evaluator_private": {
            "distributed": False,
            "server_only": True,
            "manifest_identity": _digest(f"evaluator:{episode_id}"),
        },
        "provenance": {
    ...
    lineage_axes["task_manifest"] = task_identity
    ...
    receipt = {
        ...
        "capture_backend": {
            ...
            "sensor_physics_smoke_receipt_identity": _digest("isaac-smoke"),
        },
        "integrity": {
            "online_capture": True,
            ...
            "independent_validator_identity": _digest("independent-validator"),
            "pose_closure_threshold_m": 0.01,
        },
        "partitions": {
            "policy_visible_audit_identity": _digest("policy-audit"),
    ...
    return episode_root, identity_file(receipt_path)
...
class DatasetPipelineTests(unittest.TestCase):
    def test_collector_preserves_collection_binding_in_admission_and_ledger(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(
                root / "captures",
            ...
            supply_chain = _write_release_supply_chain(
                root,
                data_receipt_identities=(receipt_identity,),
            )
            result = DatasetCollector(
                dataset_root,
                trusted_receipt_identities=[receipt_identity],
    ...
    def test_collector_records_admission_denominator_in_public_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            dataset_root = root / "dataset"
            admitted, admitted_identity = _candidate(root / "captures", episode_id="formal-episode-ledger-a")
            rejected, rejected_identity = _candidate(root / "captures", episode_id="formal-episode-ledger-b")
            supply_chain = _write_release_supply_chain(
                root,
                data_receipt_identities=(admitted_identity, rejected_identity),
            )
            collector = DatasetCollector(
                dataset_root,
                trusted_receipt_identities=[admitted_identity],
                supply_chain_manifest=supply_chain,
            )
    ...
    def test_collector_rejects_pending_or_unbound_supply_chain(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            pending = _release_supply_chain((receipt_identity,))
    ...
    def test_collector_rejects_mixed_supply_chain_before_projection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first, first_identity = _candidate(
                root / "captures",
                episode_id="formal-episode-supply-a",
            )
            second, second_identity = _candidate(
                root / "captures",
                episode_id="formal-episode-supply-b",
            )
            first_supply = _write_release_supply_chain(
                root / "supply-a",
                data_receipt_identities=(first_identity, second_identity),
            )
            ...
            first_result = DatasetCollector(
                dataset_root,
                trusted_receipt_identities=[first_identity],
            ...
            second_result = DatasetCollector(
                dataset_root,
                trusted_receipt_identities=[second_identity],
    ...
    def test_collector_rejects_supply_chain_changed_after_verification(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            changed["assets"][0]["identity"] = "9" * 16
            with patch(
                "rivermark_benchmark.audit.formal_dataset.load_supply_chain_manifest",
    ...
    def test_formal_receipt_must_bind_observation_abi_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            receipt["observation_abi_identity"] = "0" * 16
            _write_json(receipt_path, receipt)
            report = verify_candidate_episode(candidate, require_trusted_receipt=False)
    ...
    def test_release_manifest_builder_binds_verified_public_shards(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                {shard["source_capture_identity"] for shard in payload["shards"]},
                {receipt_identity},
            )
            paths = {shard["path"] for shard in payload["shards"]}
            self.assertIn("train/formal-episode-release/cameras/0/rgb.bin", paths)
            self.assertIn("train/formal-episode-release/cameras/rgb_identities.json", paths)
    ...
    def test_trusted_candidate_is_projected_and_indexed_without_private_payload(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-a", use_template_stream=True)
            dataset_root = root / "dataset"
            supply_chain = _write_release_supply_chain(
                root,
                data_receipt_identities=(receipt_identity,),
    ...
    def test_untrusted_or_pilot_like_capture_is_quarantined_not_promoted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-b")
            dataset_root = root / "dataset"
            supply_chain = _write_release_supply_chain(
                root,
                data_receipt_identities=(receipt_identity,),
    ...
    def test_lineage_overlap_between_splits_is_quarantined(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            collector = DatasetCollector(
                dataset_root,
                trusted_receipt_identities=[first_identity, second_identity],
                supply_chain_manifest=supply_chain,
            )
    ...
    def test_altered_release_payload_and_stale_index_fail_integrity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-e")
            dataset_root = root / "dataset"
            supply_chain = _write_release_supply_chain(
                root,
                data_receipt_identities=(receipt_identity,),
            ...
            release = result.episode_root
            assert release is not None
            (release / "streams" / "state.jsonl").write_text("altered\n", encoding="utf-8")
            report = verify_dataset_integrity(dataset_root)
            self.assertFalse(report.valid)
            self.assertIn("manifest_file_identity", {issue.code for issue in report.issues})
    ...
    def test_candidate_requires_complete_template_index(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-f", use_template_stream=True)
            index_path = candidate / "cameras" / "rgb_identities.json"
            index = json.loads(index_path.read_text(encoding="utf-8"))
            index["files"].pop()
            _write_json(index_path, index)
            report = verify_candidate_episode(candidate, trusted_receipt_identities=[receipt_identity])
            codes = {issue.code for issue in report.issues}
            self.assertIn("manifest_file_identity", codes)
            self.assertIn("file_identity", codes)

    def test_non_distributed_label_is_verified_but_not_published(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-g")
            label_path = candidate / "labels" / "semantic.json"
            label_identity = _write_text(label_path, '{"class":1}\n')
            manifest_path = candidate / "episode_manifest.json"
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    ...
    def test_unbound_private_directory_is_rejected_before_collection(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            candidate, receipt_identity = _candidate(root / "captures", episode_id="formal-episode-h")
