...
from rivermark_benchmark.data.dataset import (
    collect_episode,
    identity_file,
    load_pilot_episode,
)
from rivermark_benchmark.runtime import PilotRuntimeConfig, PilotSwarmRuntime
from rivermark_benchmark.train.learned import (
    LearnedWorldModelMpcCheckpointPolicy,
    TinyVlaCheckpointPolicy,
    TinyVlmGroundingCheckpointPolicy,
)
from rivermark_benchmark.train.marl import SharedMarlCheckpointPolicy, train_shared_marl
from rivermark_benchmark.train.torch_train import (
    train_vla,
    train_vlm,
    train_world_model,
)
...
class LearnedPipelineTests(unittest.TestCase):
    ...
    def test_collect_load_and_train_all_public_profiles(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for result in (vla, vlm, world):
                ...
                self.assertEqual(metadata["checkpoint_identity"], identity_file(result.checkpoint))
                self.assertEqual(metadata["source_episodes"][0]["episode_manifest_identity"], identity_file(
    ...
    def test_checkpoint_identity_alter_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_source_manifest_identity_alter_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            manifest_path = self._episode(root, teacher="action_chunk_vla_pilot", seed=901)
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["streams"][0]["identity"] = "0" * 16
