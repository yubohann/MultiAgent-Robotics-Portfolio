...
from rivermark_benchmark.score.evaluator import (
...
class EvaluatorTests(unittest.TestCase):
    def test_valid_submission_is_scored_without_truth(self) -> None:
        ...
        self.assertEqual(report.dataset_index_identity, "a" * 16)
        self.assertEqual(report.evaluator_identity, "b" * 16)
        self.assertEqual(report.code_revision, "c" * 40)
        self.assertEqual(report.checkpoint_identity, "d" * 16)
    ...
    def test_dataset_index_binding_and_resource_limits_are_strict(self) -> None:
        payload = _submission()
        issues = validate_submission(payload, expected_dataset_index_identity="c" * 16)
        self.assertIn("dataset_index_identity", {issue.code for issue in issues})
    ...
    def test_duplicate_episode_and_metric_trace_are_strict(self) -> None:
        payload = _submission()
    ...
    def test_file_report_identities_input_and_does_not_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(json.loads(output.read_text(encoding="utf-8"))["submission_identity"], report.submission_identity)
