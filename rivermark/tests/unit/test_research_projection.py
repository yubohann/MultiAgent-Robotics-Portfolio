...
from rivermark_benchmark.data.research_projection import (
    ProjectionError,
...
from tests.unit.test_formal_dataset import _candidate, _write_json
...
class ResearchProjectionTests(unittest.TestCase):
    ...
    def test_npz_projection_streams_members_without_numpy_load(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            output = root / "zarr"
            with patch("rivermark_benchmark.data.research_projection.np.load", side_effect=AssertionError("projection must stream NPY members")):
                project_episode_to_zarr(
                    candidate,
                    output,
                    stream_ids=["streamed"],
