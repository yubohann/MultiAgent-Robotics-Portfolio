...
from rivermark_benchmark.audit import isaac_validate
from rivermark_benchmark.audit.isaac_validate import (
    VALIDATION_SCHEMA,
...
from rivermark_benchmark.ops.video import identity_file
from rivermark_benchmark.pack.isaac_pack import (
    PACK_SPEC_SCHEMA,
    PACK_SPEC_SCHEMA_V2,
    pack_isaac_capture,
)
...
class IsaacPackTests(unittest.TestCase):
    def test_backend_smoke_commitment_must_come_from_capture_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "rivermark_benchmark.pack.isaac_pack.validate_isaac_capture",
                return_value=report,
            ):
    ...
    def test_v2_packer_writes_the_same_public_scene_projection_as_the_spec(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            destination = root / "candidate"
            with (
                patch("rivermark_benchmark.pack.isaac_pack.validate_isaac_capture", return_value=report),
                patch("rivermark_benchmark.pack.isaac_pack.inspect_candidate_pack_streams", return_value=candidate_streams),
                patch("rivermark_benchmark.pack.isaac_pack.validate_candidate_abi_sources", return_value=()),
            ):
                result = pack_isaac_capture(
                    capture,
                    validation,
    ...
    def test_collection_binding_is_recomputed_and_inherited(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("rivermark_benchmark.pack.isaac_pack.validate_isaac_capture", return_value=report),
                patch("rivermark_benchmark.pack.isaac_pack.load_collection_protocol", return_value={}),
                patch("rivermark_benchmark.pack.isaac_pack.resolve_collection_binding", return_value=binding),
            ):
                result = pack_isaac_capture(
                    capture,
                    validation,
    ...
    def test_collection_binding_alter_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("rivermark_benchmark.pack.isaac_pack.validate_isaac_capture", return_value=report),
                patch("rivermark_benchmark.pack.isaac_pack.load_collection_protocol", return_value={}),
                patch("rivermark_benchmark.pack.isaac_pack.resolve_collection_binding", return_value=expected),
            ):
                result = pack_isaac_capture(
                    capture,
                    validation,
    ...
    def test_projects_closed_world_candidate_and_binds_receipts(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("rivermark_benchmark.pack.isaac_pack.validate_isaac_capture", return_value=report):
