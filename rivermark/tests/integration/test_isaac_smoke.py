...
from rivermark_benchmark.isaac.isaac_smoke import (
    ISAAC_SMOKE_SCHEMA,
...
class IsaacSmokeTests(unittest.TestCase):
    ...
    def test_full_smoke_defaults_use_production_route_witness_resolution(self) -> None:
        ...
        with patch(
            "rivermark_benchmark.isaac.isaac_smoke.run_target_free_smoke", return_value={}
        ) as run_smoke:
            self.assertEqual(
                main(
                    [
    ...
    def test_smoke_rejects_disabled_foreign_process_guard_before_isaac(self) -> None:
        ...
        with self.assertRaisesRegex(IsaacSmokeError, "finite and positive"):
            from rivermark_benchmark.isaac.isaac_smoke import (
                _run_target_free_smoke_checked,
            )
    ...
    def test_running_smoke_receipt_is_terminalized_after_prelaunch_failure(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "rivermark_benchmark.isaac.isaac_smoke._run_target_free_smoke_checked",
    ...
    def test_prelaunch_commit_rejection_binds_telemetry_and_blocks_source_activation(self) -> None:
        ...
        for expected_phase, snapshots, expected_phases in (
            ...
            with (
                ...
                patch("rivermark_benchmark.isaac.isaac_smoke.ResourceTelemetry", return_value=Telemetry(snapshots)),
                patch(
                    "rivermark_benchmark.isaac.isaac_smoke.detect_source_provenance",
                    return_value=source,
                ),
                patch(
                    "rivermark_benchmark.isaac.isaac_smoke.load_runtime_lock",
                    return_value=lock,
                ),
                patch(
                    "rivermark_benchmark.isaac.isaac_smoke.runtime_lock_identity",
                    return_value="a" * 16,
                ),
                patch(
                    "rivermark_benchmark.isaac.isaac_smoke.audit_runtime_lock",
                    return_value={"status": "passed"},
                ),
                patch(
                    "rivermark_benchmark.isaac.isaac_smoke._windows_system_commit_snapshot",
                    side_effect=AssertionError("provided telemetry snapshot must be used"),
                ),
                patch("rivermark_benchmark.isaac.isaac_smoke._activate_local_isaaclab_source") as activate_source,
            ):
