...
from rivermark_benchmark.citylite_scene.citylite_task import (
    sample_private_targets,
    target_visibility_geometry_contract,
)
from rivermark_benchmark.isaac.isaac_capture import (
    AGENT_COUNT,
...
from rivermark_benchmark.isaac.isaac_runtime_safety import (
...
from rivermark_benchmark.runtime.preflight import PreflightCheck, PreflightReport
...
class IsaacCaptureBoundaryTests(unittest.TestCase):
    ...
    def test_runtime_storage_guard_fails_closed_before_external_space_is_consumed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            receipt: dict[str, object] = {}
            with patch(
                "rivermark_benchmark.isaac.isaac_capture.shutil.disk_usage",
    ...
    def test_sensor_physics_smoke_binding_is_external_exact_and_strict(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "rivermark_benchmark.isaac.isaac_smoke.validate_smoke_receipt",
                return_value=(),
            ), patch(
                "rivermark_benchmark.runtime.runtime_lock.runtime_lock_identity",
    ...
    def test_windows_commit_guard_rejects_pressure_before_app_launcher(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            receipt: dict[str, object] = {}
            with patch(
                "rivermark_benchmark.isaac.isaac_capture._windows_system_commit_snapshot",
    ...
    def test_main_preflight_rejection_binds_commit_snapshot_to_telemetry(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            telemetry = Telemetry()
            with patch(
                "rivermark_benchmark.isaac.isaac_capture.ResourceTelemetry",
                return_value=telemetry,
            ), patch(
                "rivermark_benchmark.isaac.isaac_capture._windows_system_commit_snapshot",
                side_effect=AssertionError("guard must use the telemetry snapshot"),
            ), patch("rivermark_benchmark.isaac.isaac_capture._capture") as capture:
                result = main(
                    [
                        "--output-dir",
                        str(root),
    ...
    def test_preflight_rejection_persists_receipt_without_entering_capture(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("rivermark_benchmark.runtime.preflight.run_preflight", return_value=report),
                patch(
                    "rivermark_benchmark.isaac.isaac_capture.foreign_native_process_census",
                    return_value={
                        "schema": "org.rivermark.foreign-native-process-census.v1",
                        "enumerated_native_process_count": 0,
                        "minimum_private_commit_bytes": 8 * 1024**3,
    ...
    def test_public_route_witness_rejects_a_second_pose(self) -> None:
        ...
        with patch(
            "rivermark_benchmark.isaac.isaac_capture.OVERVIEW_WITNESS_SHOTS", two_poses
    ...
    def test_public_follow_camera_authors_the_usd_render_transform(self) -> None:
        ...
        with (
            patch("rivermark_benchmark.isaac.isaac_capture._author_camera_usd_look_at") as author,
            patch(
                "rivermark_benchmark.isaac.isaac_capture._camera_usd_pose_closure_for_view",
    ...
    def test_onboard_render_preparation_uses_only_native_parent_relative_mount(self) -> None:
        ...
        with (
            patch(
                "rivermark_benchmark.isaac.isaac_capture._expected_onboard_camera_world_poses",
                return_value=(expected_positions, expected_world_quats),
            ),
            patch(
                "rivermark_benchmark.isaac.isaac_capture._synchronize_onboard_camera_fabric_local_transforms",
                side_effect=AssertionError("onboard render preparation must not write a Fabric local override"),
                create=True,
            ) as local_override,
            patch(
                "rivermark_benchmark.isaac.isaac_capture._author_onboard_camera_usd_transforms",
                side_effect=AssertionError("onboard render preparation must not author a manual USD matrix"),
                create=True,
            ) as usd_author,
            patch("rivermark_benchmark.isaac.isaac_capture._onboard_camera_usd_pose_closure", side_effect=usd_pose),
            patch("rivermark_benchmark.isaac.isaac_capture._require_onboard_camera_usd_pose", side_effect=require_usd),
        ):
