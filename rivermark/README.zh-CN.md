...
城市会藏住目标。四旋翼可能从建筑旁飞过而错过庭院，在障碍物边缘丢掉目标视线，或者在有效窗口内通过得太快。Rivermark 把这些情形记录成同步多传感器回合，每个仿真步先写入控制指令再推进状态，观测与动作的因果链保持完整。场景，采集协议，运行环境与源代码都以 SHA-256 合同绑定，回合通过独立验证后进入正式数据集。隐藏目标真值由计分器保管，搜索方法依靠飞行中的真实观测取得确认。
...
场景，协议，运行环境与源代码树都以 SHA-256 绑定。运行环境锁配置 `citylite-windows-isaacsim-5.1.0.0-local-isaaclab-2.3.2` 固定解释器，依赖版本，显卡下限以及渲染与物理配置。同种子分析器在预先声明的容差下比较同一回合种子的两次采集，并按类别与智能体编号做帧对齐。

独立验证器重新打开原始产物，检查场景身份，传感器同步，动作因果，视觉与激光雷达侵入门禁，接触，路线实现，目标可见性证据，来源与哈希绑定。通过检查的回合进入正式数据集，失败产物保留在失败账本中，长时间采集期间支持崩溃遗留恢复。
