...
def _system_commit_percent() -> float | None:
    from ..runtime.resource_telemetry import _system_commit_snapshot
...
def _make_native_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    source = detect_source_provenance(Path(__file__).resolve().parents[3])
    ...
    lease = repository_app_launcher_lease(
        ...
        metadata={
            "owner": "rivermark_benchmark.isaac.cf2x_runtime_calibration",
