...
| External models | Fail-closed adapters for selected families | A result with pinned weights, license, preprocessing, and an Isaac receipt |
...
The design supports controlled ablations across information profiles, from state-only to RGB-D to RGB-D with LiDAR and IMU to message-aware policies, and across centralized and decentralized controllers under matched observation, action, communication, and compute budgets. Any comparison reports the exact profile, model revision, checkpoint hash, training seeds, compute budget, wall time, GPU memory, failure rate, and scoring split.
