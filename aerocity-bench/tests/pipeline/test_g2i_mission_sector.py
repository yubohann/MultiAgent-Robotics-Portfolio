...
from aerocity_bench.generation.compiler import compile_g2i_task_spec, compile_method_task_spec
...
@pytest.fixture(scope="module")
def g2i_episode(ordinary_config, development_city):
...
def test_g2i_task_compiles_a_full_atlas_and_mission_sector(
    ordinary_config, development_city, g2i_episode
) -> None:
    task_spec, episode = g2i_episode
    atlas = task_spec["inspection_atlas"]
    sector = episode["mission_sector"]

    assert task_spec["task_track"] == TASK_TRACK_G2I
...
def test_sector_must_be_bound_to_its_atlas_layout(ordinary_config, g2i_episode) -> None:
    task_spec, episode = g2i_episode
    atlas = task_spec["inspection_atlas"]
...
def test_sector_rejects_an_assignment_that_drops_a_cell(ordinary_config, g2i_episode) -> None:
    task_spec, episode = g2i_episode
    atlas = task_spec["inspection_atlas"]
    forged = copy.deepcopy(episode["mission_sector"])
...
def test_sector_rejects_unknown_or_duplicate_obligations(ordinary_config, g2i_episode) -> None:
    task_spec, episode = g2i_episode
    atlas = task_spec["inspection_atlas"]
...
def test_coarse_projection_is_versioned_and_withholds_full_geometry(
    ...
    full_task = compile_g2i_task_spec(
        development_city,
        ordinary_config.raw["execution_contract"],
        ordinary_config.raw["fleet"],
    )
