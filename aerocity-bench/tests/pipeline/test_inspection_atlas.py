...
def test_g2i_is_explicit_and_does_not_reclassify_g1_u(ordinary_config, development_city) -> None:
    g1 = compile_method_task_spec(
        development_city,
        ordinary_config.raw["execution_contract"],
        ordinary_config.raw["fleet"],
    )
    g2 = compile_g2i_task_spec(
        development_city,
        ordinary_config.raw["execution_contract"],
        ordinary_config.raw["fleet"],
    )
    ...
    assert g2["schema"] == "org.aerocity.bench.task-spec-public.g2i.v1"
    assert g2["task_track"] == TASK_TRACK_G2I
...
def test_density_candidates_are_explicit_validated_and_do_not_mutate_default(
    ...
    nominal_policy = inspection_sampling_policy(
        "g2i-geometric-sampling-calibration-candidate-v2"
    )
...
def test_coarse_and_full_prior_projections_have_explicit_information_limits(
    ...
    coarse_task = compile_g2i_task_spec(
        development_city,
        ordinary_config.raw["execution_contract"],
        ordinary_config.raw["fleet"],
