...
from aerocity_bench.generation.ordinary_config import validate_public_execution_contract
...
def validate_public_episode(
    ...
    task_node = task_spec
    if task_node.get("task_track") != "G2I":
        return
    ...
    if sector is not None:
        ...
        if not isinstance(sector, dict):
            raise ValueError("G2I public episode mission sector is invalid")
...
def audit_public_layout(layout_root: Path) -> dict[str, Any]:
    ...
    for episode_path in episodes:
        ...
        if (
            ...
            raise ValueError(
                f"full-cell G2I public episode lacks its mission sector: {episode_path}"
            )
    return {
