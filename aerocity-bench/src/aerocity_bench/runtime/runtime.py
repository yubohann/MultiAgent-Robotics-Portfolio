...
from aerocity_bench.atlas.inspection_atlas import (
    validate_public_inspection_atlas,
    validate_public_mission_sector,
)
from aerocity_bench.core.canonical import derived_seed
from aerocity_bench.core.contracts import (
    ActionPacket,
    BudgetLedger,
    ExecutionReceipt,
    FailureRecord,
...
from aerocity_bench.generation.geometry import (
    Vec3,
    colliders_from_city,
    distance,
    in_field_of_view,
...
from aerocity_bench.generation.ordinary_config import OrdinaryReleaseConfig
from aerocity_bench.generation.planning_cadence import PlanningCadenceController
from aerocity_bench.release.public_boundary import validate_public_episode
from aerocity_bench.runtime.evaluator import PrivateEvaluator
...
class L0FleetRuntime:
    ...
    def _load_public_atlas_cells(
        ...
        if not isinstance(public_task_spec, dict) or public_task_spec.get("task_track") != "G2I":
            return {}
        atlas = public_task_spec.get("inspection_atlas")
        if not isinstance(atlas, dict):
            raise ValueError("G2I task spec lacks its public inspection atlas")
        ...
        if mission_sector is not None:
            if not isinstance(mission_sector, dict):
                raise ValueError("G2I episode mission sector must be an object")
            validate_public_mission_sector(
                mission_sector,
                atlas,
            ...
            if self.public_episode.get("layout_id") != mission_sector.get("layout_id"):
                raise ValueError("G2I public episode mission-sector binding differs")
            selected_cell_ids = {
        ...
        for atlas_key, active_key in observation_bindings.items():
            ...
            if not math.isclose(
                ...
                raise ValueError("G2I atlas observation contract differs from active runtime")
        ...
        for region in atlas.get("regions", []):
            for cell in region.get("cells", []):
                ...
                if not isinstance(pose_data, dict) or not isinstance(envelope, dict):
                    raise ValueError("G2I atlas cell is missing public pose envelope")
                ...
                if min(
                    ...
                    raise ValueError("G2I atlas cell pose envelope must be positive")
                cell_id = str(cell.get("cell_id", ""))
                if not cell_id or cell_id in cells:
                    raise ValueError("G2I atlas cell IDs must be unique")
        ...
        if selected_cell_ids is not None and set(cells) != selected_cell_ids:
            raise ValueError("G2I runtime mission sector does not resolve exactly")
        return cells
    ...
    def _validate_public_episode_binding(self) -> None:
        ...
        public_task_declares_g2i = (
            isinstance(self.public_task_spec, dict)
            and self.public_task_spec.get("task_track") == "G2I"
        )
        private_episode_declares_g2i = "mission_sector" in self.private_episode
        is_g2i = public_task_declares_g2i or private_episode_declares_g2i
        if not is_g2i:
            return
        if not public_task_declares_g2i:
            raise ValueError("G2I runtime requires the method-visible public task spec")
        if not isinstance(self.public_episode, dict):
            raise ValueError("G2I runtime requires the method-visible public episode")
        ...
        for binding_field in ("episode_id", "layout_id", "fleet_profile", "starts"):
            if binding_field not in self.private_episode:
                raise ValueError(f"G2I private episode lacks {binding_field}")
            if self.public_episode[binding_field] != self.private_episode[binding_field]:
                raise ValueError(f"G2I public episode binding differs for {binding_field}")
        ...
        if (public_sector is None) != (private_sector is None):
            raise ValueError("G2I public/private mission-sector presence differs")
        if public_sector is not None and public_sector != private_sector:
            raise ValueError("G2I public/private mission-sector binding differs")
