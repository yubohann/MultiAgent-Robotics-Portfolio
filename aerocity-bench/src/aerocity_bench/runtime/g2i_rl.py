"""Target-free, high-level reinforcement-learning wrappers for G2I."""
...
from aerocity_bench.core.contracts import ActionPacket, ObservationPacket
from aerocity_bench.generation.geometry import distance
from aerocity_bench.generation.planning_cadence import PlanningCadenceController
from aerocity_bench.release.public_boundary import assert_public_fields, validate_public_episode
from aerocity_bench.runtime.adapters import project_g1
from aerocity_bench.runtime.runtime import L0FleetRuntime, RuntimeStep

G2I_RL_OBSERVATION_SCHEMA = "org.aerocity.bench.g2i-rl-observation.v1"
G2I_RL_CONTEXT_SCHEMA = "org.aerocity.bench.g2i-rl-public-context.v1"
G2I_REWARD_SCHEMA = "org.aerocity.bench.g2i-rl-reward.v1"
_REWARD_COMPONENTS = (
...
class G2IRewardContract:
    ...
    def __post_init__(self) -> None:
        if not self.contract_id:
            raise ValueError("G2I reward contract ID cannot be empty")
        if set(self.weights) != set(_REWARD_COMPONENTS):
            raise ValueError("G2I reward component fields differ")
        if any(not math.isfinite(float(value)) for value in self.weights.values()):
            raise ValueError("G2I reward weights must be finite")
        if self.credit_assignment != "team_shared":
            raise ValueError("G2I v1 reward uses team-shared credit assignment")

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema": G2I_REWARD_SCHEMA,
...
G2I_INSPECTION_SHAPED_REWARD_V1 = G2IRewardContract(
    contract_id="g2i-inspection-shaped-v1",
    weights={
...
G2I_CONFIRMATION_ONLY_REWARD_V1 = G2IRewardContract(
    contract_id="g2i-confirmation-only-v1",
    weights={
...
def build_g2i_rl_public_context(runtime: L0FleetRuntime) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        or task.get("task_track") != "G2I"
        or not isinstance(episode, dict)
    ):
        raise ValueError("G2I RL wrapper requires a public G2I task and episode")
    ...
    if not isinstance(atlas, dict) or not isinstance(sector, dict):
        raise ValueError("G2I RL wrapper requires a selected public mission sector")
    ...
    if {cell_id for _, cell_id, _ in encoded} != selected_ids:
        raise ValueError("G2I RL context does not resolve the complete mission sector")
    encoded.sort(key=lambda item: item[0])
    if len({handle for handle, _, _ in encoded}) != len(encoded):
        raise ValueError("G2I RL semantic cell handles are not unique")
    context: dict[str, Any] = {
        "schema": G2I_RL_CONTEXT_SCHEMA,
    ...
    assert_public_fields(context, path="g2i_rl_context")
...
class G2IGymnasiumFleetWrapper:
    ...
    def __init__(
        ...
        *,
        reward_contract: G2IRewardContract = G2I_INSPECTION_SHAPED_REWARD_V1,
    ) -> None:
        self.runtime = runtime
        self.reward_contract = reward_contract
        self.public_context, self._cell_id_by_handle = build_g2i_rl_public_context(runtime)
    ...
    def _inspection_history(self) -> dict[str, Any]:
        ...
        return {
            "schema": "org.aerocity.bench.g2i-rl-inspection-history.v1",
    ...
    def _project(self, observations: dict[str, ObservationPacket]) -> dict[str, dict[str, Any]]:
        ...
        return {
            drone_id: {
                "schema": G2I_RL_OBSERVATION_SCHEMA,
                "public_context_ref": (
    ...
    def step(self, actions: dict[str, ActionPacket]):
        ...
        if not due:
            raise RuntimeError("G2I RL step was called before the next high-level decision")
