...
from aerocity_bench.core.contracts import ActionPacket, ObservationPacket, Pose3D
...
from aerocity_bench.release.public_boundary import (
    validate_public_episode,
    validate_public_task_spec,
)
from aerocity_bench.runtime.adapters import arbitrate_public_fleet_actions
...
def _atlas_scan_groups(
    ...
    *,
    ...
    """Turn only public G2I atlas cells into a bounded L0 route.
    ...
    if not isinstance(atlas, dict) or task_spec.get("task_track") != "G2I":
        raise ValueError("atlas-surface-inspector requires a G2I task spec")
    regions = atlas.get("regions")
    if not isinstance(regions, list) or not regions:
        raise ValueError("G2I task spec has no public inspection regions")
    selected_region_ids = (
    ...
    for region in selected:
        ...
        for cell in chosen:
            ...
            if not isinstance(pose, dict):
                raise ValueError("G2I atlas cell lacks a public pose")
            poses.append(
                Pose3D(
    ...
    if not groups:
        raise ValueError("G2I atlas has no usable public inspection cells")
    return groups
...
def _coarse_region_scan_groups(
    ...
    *,
    ...
    """Compile viewpoints from the coarse G2I region projection only.
    ...
    if not isinstance(projection, dict) or projection.get("prior_level") != ATLAS_PRIOR_COARSE:
        raise ValueError("coarse region inspector requires the coarse G2I projection")
    ...
    if not groups:
        raise ValueError("coarse G2I projection has no executable public regions")
    return groups
...
def _budgeted_atlas_region_policy(
    ...
    if not isinstance(atlas, dict):
        raise ValueError("atlas-region-greedy requires the full G2I atlas")
    selected_region_ids = (
        {str(value) for value in mission_sector["selected_region_ids"]}
        if mission_sector is not None
        else None
...
def _budgeted_atlas_surface_policy(
    ...
    atlas = task_spec.get("inspection_atlas")
    if not isinstance(atlas, dict):
        raise ValueError("atlas-surface-inspector requires the full G2I atlas")
    selected_region_ids = (
        {str(value) for value in mission_sector["selected_region_ids"]}
        if mission_sector is not None
        else None
...
def create_baseline(
    ...
    *,
    ...
    if task_spec.get("task_track") == "G2I":
    ...
    if mission_sector is not None:
        ...
        if not isinstance(mission_sector, dict) or not isinstance(atlas, dict):
            raise ValueError("public mission sector requires the full G2I atlas")
        validate_public_mission_sector(
            mission_sector,
            atlas,
