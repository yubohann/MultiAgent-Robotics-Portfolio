...
from aerocity_bench.core.canonical import read_json
from aerocity_bench.core.contracts import ActionPacket, MessagePacket, ObservationPacket, Pose3D
from aerocity_bench.generation.geometry import segment_segment_distance
...
class ExternalL1AdapterManifest:
    ...
    def validate(self) -> None:
        ...
        if self.declaration.capability_profile != "G2I":
            raise ValueError("external L1 manifests require the G2I capability profile")
...
class PlannerAdapter:
    ...
    def __init__(self, declaration: AdapterDeclaration, planner: Planner) -> None:
        ...
        if declaration.capability_profile not in {"G1", "G2I"}:
            raise ValueError("PlannerAdapter implements canonical G1 or G2I observations")
        ...
        *,
        ...
        if self.declaration.capability_profile == "G2I":
            if (
                ...
                or public_task_spec.get("task_track") != "G2I"
            ):
                raise ValueError("G2I adapter reset requires a public G2I task spec")
            validate_public_episode(public_episode, public_task_spec)
            _assert_external_wire_is_public(public_task_spec)
            self.planner.reset(
                {
                    "schema": "org.aerocity.bench.g2i-planner-reset.v1",
        ...
        if public_task_spec is not None:
            raise ValueError("G1 adapter reset must not receive a G2I task spec")
...
class ExternalProcessPlannerBridge:
    """Run a centralized G1/G2I planner through a persistent JSONL process.
    ...
    def __init__(
        ...
        *,
        ...
        if declaration.capability_profile not in {"G1", "G2I"}:
            raise ValueError(
                "ExternalProcessPlannerBridge implements canonical G1 or G2I observations"
            )
        ...
        *,
        ...
        if self.declaration.capability_profile == "G2I":
            if (
                ...
                raise ValueError("G2I process reset requires a public G2I task spec")
            validate_public_episode(public_episode, public_task_spec)
            payload["public_task_spec"] = _external_process_episode_projection(
                public_task_spec
            )
        elif public_task_spec is not None:
            raise ValueError("G1 process reset must not receive a G2I task spec")
...
def _reject_unsupported_g2i_rl_wrapper(runtime: L0FleetRuntime) -> None:
    task_spec = runtime.public_task_spec
    if isinstance(task_spec, dict) and task_spec.get("task_track") == "G2I":
        raise ValueError(
            "the legacy RL wrapper exposes only the G1 observation projection and "
            "confirmation-only reward; use a versioned G2I training wrapper before training"
        )
...
class GymnasiumFleetWrapper:
    ...
    def __init__(self, runtime: L0FleetRuntime) -> None:
        _reject_unsupported_g2i_rl_wrapper(runtime)
