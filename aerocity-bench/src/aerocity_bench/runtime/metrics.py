...
from aerocity_bench.bridge.isaac_bridge import FormalExecutionContext, assert_formal_receipts
from aerocity_bench.core.errors import ValidationError
...
def evaluate_run(
    ...
    *,
    ...
    if (inspection_cell_denominator > 0) != (inspection_area_denominator > 0.0):
        raise ValueError("G2I run has inconsistent inspection denominators")
    if inspection_area_denominator > 0.0 and (
        not inspection_trace or not inspection_cell_trace
    ):
        raise ValueError("G2I run lacks an inspection-footprint coverage trace")
