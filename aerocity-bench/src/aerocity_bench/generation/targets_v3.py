...
# distribution.
_G2I_PRIVATE_SUPPORT_POLICY = {
    "policy_id": "g2i-private-surface-support-v1",
...
def _g2i_private_surface_sites(city: dict[str, Any]) -> list[dict[str, Any]]:
    """Compile a density-independent private support lattice from CitySpec."""

    spacing = float(_G2I_PRIVATE_SUPPORT_POLICY["surface_spacing_m"])
    side_inset = float(_G2I_PRIVATE_SUPPORT_POLICY["facade_inset_m"])
    height_inset = float(_G2I_PRIVATE_SUPPORT_POLICY["facade_height_inset_m"])
    offset = float(_G2I_PRIVATE_SUPPORT_POLICY["surface_offset_m"])
    sites: list[dict[str, Any]] = []
...
def _mission_sector_support_sites(
    ...
    if not isinstance(atlas, dict):
        raise GenerationRejected("G2I mission sampling requires the full public atlas")
    validate_public_mission_sector(
    ...
    result: list[dict[str, Any]] = []
    for raw_site in _g2i_private_surface_sites(city):
        matching_regions = [
            region
...
def sample_episode_v3(
    ...
    *,
    ...
    if (precomputed_mission_sector is None) != (
        ...
        raise ValueError("precomputed G2I sector and sites must be supplied together")
    if precomputed_mission_sector is not None and public_task_spec is None:
        raise ValueError("precomputed G2I sector requires a public task spec")
    ...
    if public_task_spec is not None:
        if public_task_spec.get("task_track") != TASK_TRACK_G2I:
            raise ValueError("mission-sector sampling requires a public G2I task spec")
        atlas = public_task_spec.get("inspection_atlas")
        if not isinstance(atlas, dict):
...
def validate_frozen_g2i_episode(
    ...
    if episode.get("schema") != "org.aerocity.bench.episode-private.ordinary.v3":
        raise ValueError("frozen G2I episode schema differs")
    if task_spec.get("task_track") != TASK_TRACK_G2I:
        raise ValueError("frozen G2I episode requires a public G2I task spec")
    atlas = task_spec.get("inspection_atlas")
    if not isinstance(atlas, dict):
        raise ValueError("frozen G2I episode requires the authority full-cell atlas")
    if episode.get("layout_id") != city.get("layout_id"):
        raise ValueError("frozen G2I episode is not bound to its city")
    sector = episode.get("mission_sector")
    if not isinstance(sector, dict):
        raise ValueError("frozen G2I episode lacks its public mission sector")
    validate_public_mission_sector(
        sector,
        atlas,
        episode.get("starts"),
