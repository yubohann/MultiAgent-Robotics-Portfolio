...
from aerocity_bench.core.canonical import derived_seed, write_json
from aerocity_bench.generation.ordinary_config import public_execution_contract
from aerocity_bench.release.assets import AssetLock
from aerocity_bench.release.public_boundary import validate_public_task_spec
...
def compile_method_task_spec(
    ...
    *,
    ...
    exploration compatibility.  ``G2I`` is an explicit opt-in compiler path:
    ...
    if task_track not in {TASK_TRACK_G1_U, TASK_TRACK_G2I}:
    ...
    if task_track == TASK_TRACK_G1_U and inspection_prior_level != ATLAS_PRIOR_FULL:
        raise ValueError("G1-U does not accept a G2I inspection prior level")
    ...
    if task_track == TASK_TRACK_G2I:
        ...
        task_spec["schema"] = "org.aerocity.bench.task-spec-public.g2i.v1"
        task_spec["task_track"] = TASK_TRACK_G2I
        ...
        if inspection_prior_level == ATLAS_PRIOR_FULL:
            ...
            raise ValueError(f"unsupported G2I inspection prior: {inspection_prior_level}")
    validate_public_task_spec(task_spec)
    return task_spec


def compile_g2i_task_spec(
    city: dict[str, Any],
    execution_contract: dict[str, Any],
    fleet_profile: dict[str, Any],
    *,
    ...
    """Compile the planned G2I public projection without changing G1-U output."""

    return compile_method_task_spec(
        city,
        execution_contract,
        fleet_profile,
        task_track=TASK_TRACK_G2I,
