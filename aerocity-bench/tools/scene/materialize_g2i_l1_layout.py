"""Materialize one development-only G2I layout for native CF2X replay.

This is deliberately separate from the official release builder.  It binds a
development CitySpec and one private episode to the public G2I task contract,
...
from aerocity_bench.generation.compiler import compile_g2i_task_spec, write_compiled_public_v3
...
from aerocity_bench.generation.targets_v3 import (
    ...
    sample_episode_v3,
    validate_frozen_g2i_episode,
)
from aerocity_bench.release.assets import stage_assets
from aerocity_bench.release.public_boundary import audit_public_layout
...
def materialize(
    ...
    *,
    ...
    if development_split is not None and development_split in FORMAL_SPLITS:
        raise ValueError("native G2I development layout cannot consume a formal split")
    ...
    try:
        ...
        authority_task = compile_g2i_task_spec(
            city,
        ...
        write_json(output / "development_layout_manifest.json", {
            "schema": "org.aerocity.bench.g2i-development-layout.v1",
