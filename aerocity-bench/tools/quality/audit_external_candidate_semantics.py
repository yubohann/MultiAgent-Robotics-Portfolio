"""Fail closed if a screened external candidate is misrepresented as G2I-ready."""
...
def audit(path: Path) -> dict[str, Any]:
    ...
    public_inputs = raw["g2i_public_inputs"]
    forbidden_inputs = raw["g2i_forbidden_inputs"]
    candidates = raw["candidates"]
    if not all(isinstance(value, str) and value for value in public_inputs):
        raise ValueError("G2I public inputs must be non-empty strings")
    if not all(isinstance(value, str) and value for value in forbidden_inputs):
        raise ValueError("G2I forbidden inputs must be non-empty strings")
    ...
    return {
        ...
        "status": "BLOCKED_NO_SUBSTANTIVE_EXTERNAL_G2I_METHOD"
        if not eligible
