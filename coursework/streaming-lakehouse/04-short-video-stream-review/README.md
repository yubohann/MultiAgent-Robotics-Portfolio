...
| 路径 | 说明 |
| --- | --- |
| `app/` | FastAPI + SQLite + Ollama VLM 短视频审核 Demo |
| `data/incoming` | 4 个小体积真实短视频样例 |
| `LAB_README.md` | Demo 运行说明与模型候选 |
| `screenshots_[REDACTED]` | 环境，模型，API，页面和 Kafka 截图 |
| `logs_[REDACTED]` | 环境，模型，验收，API，事件流和 Kafka 日志 |
| `短视频审核实验报告_REDACTED_余博涵.docx` | 最终 Word 版实验报告 |
...
运行前需要本机已安装 FFmpeg/FFprobe，并已在 Ollama 中准备 `ministral-3:8b` 等模型。提交包保持精简，FFmpeg 二进制和 Ollama 模型文件由本机环境提供。
...
GitHub 提交包保持精简，本地 `.venv`，FFmpeg 二进制包，运行缓存和 SQLite 临时状态留在本机。原始 128 MB 无人机视频替换为压缩后的真实样例视频，便于助教直接确定性回放。
