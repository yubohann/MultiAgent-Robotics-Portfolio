...
默认模式使用 `SQLite jobs` 模拟工业系统中的消息队列，开箱即可跑通。课堂环境具备 Kafka 时，可以使用 `scripts/kafka_*` 脚本把同一条链路切到真实消息队列。

## 快速运行

建议使用 Python 3.11 或 3.12。Python 3.14 生态里部分科学计算依赖的 wheel 可能仍在完善中。

```bash
cd 04-short-video-stream-review
...
上传视频时，网站会先把真实视频文件写入媒体区并显示在信息流中，状态为 `处理中`。后台再异步执行理解，打标签和审核。等待期间只在摘要和标签位置显示 loading 占位符，视频播放器本身保持可见。

为了兼顾学生电脑可运行和工业架构思维，本实验默认采用工业轻量版
...
| `local-baseline` | OpenCV Local Baseline | CPU 环境兜底方案 |

## 下载本地模型

先安装并启动 Ollama，下载地址 https://ollama.com/download。Qwen3-VL 需要 Ollama 0.12.7 或更新版本，下载脚本会自动检查版本。
