...
- Final Word report, `短视频审核实验报告_REDACTED_余博涵.docx`
- Source manual and lab README, `instruction.md`, `README.md`
- FastAPI and Ollama demo code under `app/`, `scripts/`, `static/` and `templates/`
...
The FastAPI workflow and model comparison logs cover all three real demo categories. The Kafka bonus path records one drone-video ingest event and one processed result message, so the Kafka result log contains a single message, and the API and baseline logs cover the other videos.
