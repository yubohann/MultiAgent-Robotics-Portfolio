...
独立完成四个实验的搭建与运行。湖仓实验：Docker Compose 环境，Flink 与 Dinky 作业，Paimon 与 MinIO 存储，Spark 批处理核对（`01-modern-lakehouse/`）。挑战实验：七类流处理问题复现与前后对比证据（`02-streaming-challenges/`）。推荐实验：Flink 实时特征作业，服务层与融合输出（`03-ai-recommender/`）。短视频审核实验：FastAPI 审核服务，本地 VLM 模型对比，Kafka 链路与提交包（`04-short-video-stream-review/`）。报告，截图与证据日志随各实验目录提供。
