...
> English overview in [README.en.md](README.en.md)

本文档按课程指导书中的实验五，实验六流程编写，但把原来的 YOLOv8 全部替换为 YOLO26。你按顺序执行即可，实验五完成环境，标注数据，训练。实验六完成验证，预测，导出 ONNX。

官方参考

- YOLO26 模型文档https://docs.ultralytics.com/models/yolo26/
- Ultralytics 快速开始https://docs.ultralytics.com/quickstart/
- 模型导出文档https://docs.ultralytics.com/modes/export/
...
```text
Windows:
exp5_6_yolo26\screenshots\实验五
exp5_6_yolo26\screenshots\实验六

Ubuntu:
~/yubohan_[REDACTED]_exp5_6_yolo26/screenshots/实验五
~/yubohan_[REDACTED]_exp5_6_yolo26/screenshots/实验六
```
...
`nvidia-smi` 能看到显卡时，后续训练命令使用 `--device 0`。显卡信息为空时，后续训练命令使用 `--device cpu`。
...
如果 `labelimg` 安装失败，可直接跳过，用已有标注文件继续实验。
...
标签第一列必须从 `0` 开始，最大编号至多等于 `classes.txt` 行数减 1。
