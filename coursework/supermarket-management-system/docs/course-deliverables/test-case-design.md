...
| TC02 | 登录失败 | 输入错误密码 | 提交登录 | 页面提示失败，用户会话保持未登录状态 | `tests/test_auth_and_routes.py` |
...
| TC12 | 角色权限控制 | 未登录或收银员登录 | 访问管理页面 | 访客跳转登录，收银员访问被拒绝 | `tests/test_auth_and_routes.py` |
...
当前覆盖率统计范围为 `app.models` 与 `app.services.second_phase`，对应课程二期核心服务和后端数据模型。`coverage report` 已设置 `fail_under = 100`，低于 100% 会直接失败。当前命令行记录见 `reports/system-analysis-design/screenshots/coverage-report.txt`，结果为 `TOTAL 530 0 100%`。
