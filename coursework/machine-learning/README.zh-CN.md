...
独立完成十个经典算法的实现，从数值核心到命令行入口（`classic-ml-algorithms/*/src/`），并在仓库内数据集上运行实验，编写每个算法的实验报告。独立完成作业三的完整流程：数据加载与固定划分预处理，从零实现的向量化 kNN 与均值阈值 ID3，PCA 与 LDA 运行，以及 `ml-assignment-3/results/` 中的图表与比较结果。
