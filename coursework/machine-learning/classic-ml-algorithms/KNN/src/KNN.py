...
import argparseimport csvimport mathimport osimport randomfrom collections import Counterclass KNN:
    ...
    def _distance(self, x1, x2):
        """Compute distance with the selected metric."""
    ...
    def fit(self, X, y):
        """Store the training data, since kNN trains lazily."""
        if len(X) != len(y):
    ...
    def predict_proba_single(self, x):
        """Predict the class probabilities of one sample."""
        if self.X_train is None:
            raise ValueError("模型未训练，请先调用 fit()")
    ...
    def predict_proba(self, X):
        """Predict the class probabilities of many samples."""
        probabilities = []
        for x in X:
    ...
    try:
        with open(file_path) as f:
            ...
            for row in reader:
                ...
                try:
                    # The first four columns are features.
...
def normalize_features(X, X_mean=None, X_std=None):
    """Standardize features with z-score normalization."""
    n_features = len(X[0])
