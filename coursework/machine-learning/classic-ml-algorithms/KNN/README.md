# K 近邻，即 K-Nearest Neighbors, KNN算法实现

## 1. 算法原理

K 近邻，即 KNN是一种简单而有效的分类算法，属于**懒惰学习**，即 Lazy Learning范畴。它的核心思想是，**一个样本的类别由其最近邻的 K 个样本的多数类别决定**。
...
#### 1. 欧几里得距离，即 Euclidean Distance
$$d(x_1, x_2) = \sqrt{\sum_{i=1}^{n}(x_{1i} - x_{2i})^2}$$

**特点**，最常用，对所有维度平等对待

#### 2. 曼哈顿距离，即 Manhattan Distance
$$d(x_1, x_2) = \sum_{i=1}^{n}|x_{1i} - x_{2i}|$$

**特点**，计算速度快，在高维空间表现较好

#### 3. 切比雪夫距离，即 Chebyshev Distance
...
| `-k` | - | int | 5 | 近邻数量 |
| `--metric` | `-m` | str | euclidean | 距离度量，即 euclidean/manhattan/chebyshev |
...
**经验法则**
- **K 太小**，例如 K=1，过拟合，受噪声影响大
- **K 太大**，例如 K=N，欠拟合，类别分界过于平滑
...
**解决方案**
1. **独热编码**，即 One-Hot Encoding
2. **哈明距离**，即 Hamming Distance，计算不同维度的个数
