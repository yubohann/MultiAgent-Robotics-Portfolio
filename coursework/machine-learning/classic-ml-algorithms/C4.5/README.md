...
### 关键特点
- **分裂准则**，使用**信息增益率，即 Gain Ratio**代替 ID3 的信息增益
  - 信息增益率 = 信息增益 / 分裂信息
  - 在多值特征上保持平衡
  
- **支持连续特征**，通过二分裂，阈值分裂处理连续数值特征
...
| **缺失值** | 无 |
| **类别** | 2，良性 B / 恶性 M |
| **类别分布** | 357 良性，即 B, 212 恶性，即 M |

### 特征说明

数据基于**细针穿刺，即 Fine Needle Aspirate, FNA**图像的数字化，用于诊断乳腺肿瘤。

对于每个细胞核，计算以下 **10 个基础特征**的**均值，即 mean，标准误，即 SE，最大值，即 worst**，共 30 个特征
...
该数据集用于通过机器学习诊断乳腺癌。已有研究表明，使用前 3 个特征，Worst Area，Worst Smoothness，Mean Texture的分离平面可以达到 **97.5% 的准确率**。
...
| 特性 | C4.5 实现 | sklearn |
|------|---------|---------|
| 分裂准则 | 信息增益率，即 Gain Ratio | Gini 或熵 |
