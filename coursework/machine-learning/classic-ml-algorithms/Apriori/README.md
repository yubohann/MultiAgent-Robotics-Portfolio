...
Apriori 是一种经典的频繁项集挖掘算法，用于发现数据中的关联规则。它基于一个关键假设，**频繁项集的子集也必定是频繁项集**，即先验原理。
...
#### 辅助函数
- `apriori_gen()` 用于候选集生成，类似于 Apriori 的连接步骤
...
- **纯标准库实现**，依赖清单为 Python 内置模块，pandas 与 scikit-learn 等用于对照版本
...
| `--min_support` | `-s` | float | 0.01 | 最小支持度，取值 0 到 1 |
| `--min_confidence` | `-c` | float | 0.3 | 最小置信度，取值 0 到 1 |
...
### 典型运行结果 min_support=0.01 与 min_confidence=0.3
...
**经验法则**
- **高频商品**，事务占比 50% 以上，min_support = 0.05-0.1
- **普通商品**，事务占比 10% 到 50%，min_support = 0.01-0.05
- **低频商品**，事务占比 10% 以下，min_support = 0.001-0.01
...
**业务应用**
- **促销活动**，选择 confidence > 0.4，对应较强相关性
- **推荐系统**，可用 confidence > 0.3，覆盖更多选项
...
**问题**，热销商品，例如牛奶，会出现在许多规则中，而小众商品进入频繁项集的次数较少。
