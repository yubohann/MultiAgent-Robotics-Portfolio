...
- `purity_score(y_true, y_pred)` 计算纯度，用于聚类质量计分
- `euclidean_distance()`: 欧氏距离计算

### 依赖

- 标准库`csv`, `math`, `random`, `argparse`, `os`, `collections`
- **纯标准库实现**，NumPy 与 scikit-learn 等用于对照版本
