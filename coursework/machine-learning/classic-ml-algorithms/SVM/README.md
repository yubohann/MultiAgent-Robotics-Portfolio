# SVM，基于 Pegasos 的线性 SVM

本目录包含一个纯标准库实现的线性 SVM，使用 Pegasos，即随机子梯度下降，作为优化器，并通过 one-vs-rest 策略支持多分类，用于 Iris 数据集示例。
...
- 使用 Pegasos，随机子梯度下降训练线性 SVM，hinge loss + L2 正则化
- 对于多类问题，即 Iris，采用 one-vs-rest，为每个类别训练一个二元分类器，预测时选择评分最高的类别
...
- **基本情况**
  ...
  - 属性数，特征数，4
    ...
    - petal length in cm
    - petal width in cm
  - 类别数，3，即 Iris-setosa, Iris-versicolor, Iris-virginica
  - 数据格式，CSV，每行 5 列，前 4 列为浮点数特征，第 5 列为类别标签

- **备注**，本实现会尝试在 `SVM/data/iris.data` 找到数据文件，启用 `--sample` 时脚本会尝试从 UCI 自动下载。

## 使用示例

1. 使用内置样例，自动下载 Iris 数据集到 `SVM/data/iris.data`
