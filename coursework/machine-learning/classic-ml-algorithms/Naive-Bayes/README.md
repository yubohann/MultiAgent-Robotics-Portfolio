# 朴素贝叶斯，即 Naive Bayes文本分类实现

## 1. 算法原理

朴素贝叶斯，即 Naive Bayes是一种基于贝叶斯定理的概率分类算法，广泛应用于文本分类，垃圾邮件检测，情感分析等任务。
...
**独立性假设**，所有特征，词在给定类别的条件下相互独立。

$$P(D|C) = P(w_1, w_2, \ldots, w_n|C) = \prod_{i=1}^{n} P(w_i|C)$$

虽然这个假设在现实中很少成立，朴素的由来，但该算法在实践中表现良好。

### 多项式模型，即 Multinomial Naive Bayes
...
其中
- **$\alpha$**，拉普拉斯平滑系数，通常为 1，用于平滑零计数情形
- **$|V|$**，词表大小
...
- **纯标准库实现**，scikit-learn 与 nltk 等用于对照版本
...
直接计算概率会导致数值下溢，即 extremely small numbers
...
**朴素** 指的是算法采用一个简化假设，**所有特征在给定类别的条件下相互独立**。

现实中，词与词之间通常有关联。例如，机器和学习经常一起出现，但在计算时我们假设它们独立。

**值得注意的是**，这个假设虽然简化，朴素贝叶斯在实践中仍表现良好。

### Q2: 什么是拉普拉斯平滑？为什么需要它？

**问题**，当某个词在某类文档中计数为 0 时，该词对该类的条件概率为 0

$$P(w|C) = \frac{0}{\text{total}} = 0$$

这会导致整个后验概率为 0，文本分类失效。
