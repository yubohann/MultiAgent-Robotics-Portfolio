...
AdaBoost 全称 Adaptive Boosting，是一种集成学习算法，通过迭代训练多个弱分类器，根据分类错误重新分配样本权重，最终将所有弱分类器加权组合成强分类器。
...
| **缺失值** | 无 |
| **类别** | 2 类，伽马与强子 |
| **类别分布** | 比例偏斜，约 64% g，36% h |

### 特征说明

数据来自 **MAGIC** 望远镜，全称 Major Atmospheric Gamma Imaging Cherenkov，用于检测伽马射线
...
- **纯标准库实现**，依赖清单为 Python 内置模块，scikit-learn 等第三方库用于对照版本
...
| `--n_estimators` | `-n` | int | 50 | 决策桩弱分类器数量 |
| `--test_size` | `-t` | float | 0.3 | 测试集比例，取值 0 到 1 |
