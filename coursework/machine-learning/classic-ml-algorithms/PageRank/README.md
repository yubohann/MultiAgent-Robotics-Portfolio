...
- **$d$**，阻尼因子，即 damping factor，通常为 0.85
- **$B_A$**，所有指向 A 的网页的集合
- **$C(T)$**，网页 T 的出链数量
...
| **PageRank 计算** | 迭代法，即 Power Iteration |
| **收敛判断** | 相邻迭代的最大差异 |
| **大图处理** | 支持流式加载 |
...
- **纯标准库实现**，networkx 与 numpy 等用于对照版本
...
| `--damping` | `-d` | float | 0.85 | 阻尼因子，取值 0-1 |
...
### 处理悬挂节点，即 Dangling Nodes

悬挂节点是出度为 0 的节点，例如 PDF 文件。

**方法 1，直接跳过，本实现**
- 悬挂节点向其他节点传递的 PageRank 为 0
