...
/target_detection  --------------------+
                                       |
Nav2 /navigate_to_pose action <--- behavior/competition_behavior
                                       |
/cmd_vel ------------------------------+
                                       |
/shooter/enable / /fire / /disable <---+
        |
        v
shooter/shooter_controller -> serial laser module
```
...
- `bringup` starts the complete competition stack.
- `description` publishes the robot model and TF frames.
- `navigation` stores Nav2, slam_toolbox, map and target route configuration.
- `vision` detects AprilTag Tag36h11 targets from the camera stream.
- `shooter` controls the fixed 5V laser module through parameterized serial commands.
- `behavior` owns the competition state machine and keeps perception code separate from control code.
- `interfaces` defines `TargetDetection`.
...
| `/target_detection` | `interfaces/TargetDetection` | `vision` | Structured target observation |
| `/cmd_vel` | `geometry_msgs/Twist` | `behavior`, Nav2 | Base velocity control |
| `/shooter/enable` | `std_srvs/Trigger` | `shooter` | Enable the laser module |
| `/shooter/fire` | `std_srvs/Trigger` | `shooter` | Send the firing command |
| `/shooter/disable` | `std_srvs/Trigger` | `shooter` | Disable the laser module |
| `navigate_to_pose` | `nav2_msgs/NavigateToPose` | Nav2 | Navigate to a configured target pose |

## Competition Config

- `navigation/config/targets.elimination.yellow.yaml` keeps yellow on the yellow side with blue-side targets only.
- `navigation/config/targets.elimination.blue.yaml` keeps blue on the blue side with yellow-side targets only.
- `bringup/config/sim2real.yaml` holds field, robot, sensor, shooter and randomization parameters for real-world calibration.
