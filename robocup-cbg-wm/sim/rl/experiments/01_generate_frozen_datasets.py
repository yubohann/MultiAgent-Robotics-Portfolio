...
from selfplay_env import AGENTS, RoboCupVisionRLSelfPlayEnv
from world_model import BeliefTracker, extract_rule_risks

_paired_interventions = import_module("experiments.01_paired_interventions")
generate_paired_intervention = _paired_interventions.generate_paired_intervention
_scenario_protocol = import_module("experiments.01_scenario_protocol")
SCENARIOS = _scenario_protocol.SCENARIOS
aggressive_action = _scenario_protocol.aggressive_action
apply_scenario = _scenario_protocol.apply_scenario
tracker_overrides = _scenario_protocol.tracker_overrides
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "sim" / "output" / "paper" / "cbg_wm_2026" / "frozen_data")
