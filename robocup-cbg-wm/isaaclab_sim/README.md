...
For the two robot first-person videos, repeat with `--record_view yellow_pov` and `--record_video <repo-root>\docs\media\最终回放_黄车第一视角.mp4`, then with `--record_view blue_pov` and `--record_video <repo-root>\docs\media\最终回放_蓝车第一视角.mp4`.
