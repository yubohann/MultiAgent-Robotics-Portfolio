...
WSL note. Build from a native Linux path such as `~/ros_ws`, because ROSIDL generation requires an ASCII path.

Package layout.

- `bringup` holds system launch files and top-level runtime wiring.
- `navigation` stores Nav2, slam_toolbox, maps and target route configuration.
- `motion` records motion telemetry and map-drift data.
- `vision` detects AprilTag Tag36h11 targets from camera images.
- `shooter` controls the serial laser module.
- `behavior` runs the competition state machine.
- `description` publishes the robot URDF and frame description.
- `interfaces` defines custom ROS2 message definitions.
- `docs` collects project documentation used for portfolio submission.
